    .db F0
    .db B3
    .db 03
    .db 0C
    .db F0
    .db B5
    .db 03
    .db 14
    .db F0
    .db B7
    .db 03
    .db 1C
    .db F0
    .db B9
    .db 03
    .db 24
    .db 00
    .db B9
    .db C3
    .db E4
    .db 00
    .db B7
    .db C3
    .db EC
    .db 00
    .db A9
    .db 03
    .db F4
    .db 00
    .db BB
    .db 03
    .db FC
    .db 00
    .db BD
    .db 03
    .db 04
    .db 00
    .db BF
    .db 03
    .db 0C
    .db 00
    .db AD
    .db C3
    .db 14
    .db 0A
    .db D0
    .db 81
    .db 03
    .db F0
    .db D0
    .db 83
    .db 03
    .db F8
    .db D0
    .db 85
    .db 03
    .db 00
    .db D0
    .db 87
    .db 03
    .db 08
    .db E0
    .db 89
    .db 03
    .db F8
    .db E0
    .db 8B
    .db 03
    .db 00
    .db E0
    .db 8D
    .db 03
    .db 08
    .db F0
    .db 8F
    .db 03
    .db F3
    .db F0
    .db 91
    .db 03
    .db FB
    .db F0
    .db 93
    .db 03
    .db 03
    .db 06
    .db D8
    .db 95
    .db 03
    .db F0
    .db D8
    .db 97
    .db 03
    .db F8
    .db D8
    .db 99
    .db 03
    .db 00
    .db DD
    .db 9B
    .db 03
    .db 08
    .db E8
    .db 9D
    .db 03
    .db F8
    .db E8
    .db 9F
    .db 03
    .db 00
    .db 09
    .db C0
    .db A1
    .db 03
    .db E0
    .db C6
    .db A3
    .db 03
    .db E8
    .db C6
    .db A5
    .db 03
    .db F0
    .db C8
    .db A7
    .db 03
    .db FB
    .db D0
    .db A9
    .db 03
    .db 10
    .db E0
    .db AB
    .db 03
    .db FF
    .db EB
    .db AD
    .db 03
    .db 10
    .db F0
    .db AF
    .db 03
    .db F8
    .db F0
    .db B1
    .db 03
    .db 00
    .db 0A
    .db D8
    .db 81
    .db 03
    .db F8
    .db D8
    .db 83
    .db 03
    .db 00
    .db D8
    .db 85
    .db 03
    .db 08
    .db E0
    .db 87
    .db 03
    .db 10
    .db F0
    .db 89
    .db 03
    .db E8
    .db F0
    .db 8B
    .db 03
    .db F0
    .db F0
    .db 8D
    .db 03
    .db F8
    .db F0
    .db 8F
    .db 03
    .db 00
    .db F0
    .db 91
    .db 03
    .db 08
    .db F0
    .db 93
    .db 03
    .db 10
    .db 0A
    .db D8
    .db 81
    .db 03
    .db F9
    .db D8
    .db 83
    .db 03
    .db 01
    .db D8
    .db 85
    .db 03
    .db 09
    .db E0
    .db 87
    .db 03
    .db 11
    .db F0
    .db 89
    .db 03
    .db E9
    .db F0
    .db 8B
    .db 03
    .db F1
    .db F0
    .db 8D
    .db 03
    .db F9
    .db F0
    .db 95
    .db 03
    .db 01
    .db F0
    .db 97
    .db 03
    .db 09
    .db F0
    .db 93
    .db 03
    .db 11
    .db 0A
    .db DB
    .db E3
    .db 02
    .db F0
    .db DB
    .db E3
    .db 42
    .db F8
    .db D8
    .db 81
    .db 03
    .db F8
    .db D8
    .db 83
    .db 03
    .db 00
    .db D8
    .db 85
    .db 03
    .db 08
    .db E0
    .db 87
    .db 03
    .db 10
    .db F0
    .db 89
    .db 03
    .db E8
    .db F0
    .db 8B
    .db 03
    .db F0
    .db F0
    .db 8D
    .db 03
    .db F8
    .db F0
    .db 8F
    .db 03
    .db 00
    .db F0
    .db 91
    .db 03
    .db 08
    .db F0
    .db 93
    .db 03
    .db 10
    .db 08
    .db D0
    .db 81
    .db 03
    .db F8
    .db D0
    .db 83
    .db 03
    .db 00
    .db D0
    .db 85
    .db 03
    .db 08
    .db D8
    .db 87
    .db 03
    .db 10
    .db EC
    .db 99
    .db 03
    .db F8
    .db EB
    .db 9B
    .db 03
    .db 00
    .db ED
    .db 9D
    .db 03
    .db 08
    .db E8
    .db 9F
    .db 03
    .db 10
    .db 06
    .db E0
    .db A7
    .db 03
    .db 10
    .db F0
    .db A1
    .db 03
    .db F8
    .db EE
    .db A3
    .db 03
    .db 00
    .db ED
    .db A5
    .db 03
    .db 08
    .db F0
    .db A9
    .db 03
    .db 10
    .db F0
    .db AB
    .db 03
    .db 18
    .db 0A
    .db C0
    .db AD
    .db 03
    .db 10
    .db D0
    .db AF
    .db 03
    .db 13
    .db E0
    .db B1
    .db 03
    .db 10
    .db E0
    .db B3
    .db 03
    .db 18
    .db F0
    .db 89
    .db 03
    .db E8
    .db F0
    .db 8B
    .db 03
    .db F0
    .db F0
    .db 8D
    .db 03
    .db F8
    .db F0
    .db 8F
    .db 03
    .db 00
    .db F0
    .db 91
    .db 03
    .db 08
    .db F0
    .db 93
    .db 03
    .db 10
    .db 0A
    .db D0
    .db B5
    .db 03
    .db 08
    .db D1
    .db B7
    .db 03
    .db 10
    .db E0
    .db B9
    .db 03
    .db 00
    .db E0
    .db BB
    .db 03
    .db 08
    .db F0
    .db 89
    .db 03
    .db E8
    .db F0
    .db 8B
    .db 03
    .db F0
    .db F0
    .db 8D
    .db 03
    .db F8
    .db F0
    .db 8F
    .db 03
    .db 00
    .db F0
    .db BD
    .db 03
    .db 08
    .db F0
    .db BF
    .db 03
    .db 10
    .db 02
    .db F8
    .db E1
    .db 02
    .db F8
    .db F8
    .db E1
    .db 02
    .db 00
    .db 09
    .db D0
    .db 81
    .db 03
    .db EF
    .db D0
    .db 83
    .db 03
    .db F7
    .db D0
    .db 85
    .db 03
    .db FF
    .db E0
    .db 87
    .db 03
    .db F1
    .db E0
    .db 89
    .db 03
    .db F9
    .db E0
    .db 8B
    .db 03
    .db 01
    .db F0
    .db 8D
    .db 03
    .db F4
    .db F0
    .db 8F
    .db 03
    .db FC
    .db F0
    .db 91
    .db 03
    .db 04
    .db 08
    .db CF
    .db 81
    .db 03
    .db EF
    .db CF
    .db 83
    .db 03
    .db F7
    .db CF
    .db 85
    .db 03
    .db FF
    .db DF
    .db 93
    .db 03
    .db F1
    .db DF
    .db 95
    .db 03
    .db F9
    .db DF
    .db 97
    .db 03
    .db 01
    .db EF
    .db 99
    .db 03
    .db F9
    .db EF
    .db 9B
    .db 03
    .db 01
    .db 06
    .db E0
    .db B9
    .db 03
    .db F4
    .db E0
    .db BB
    .db 03
    .db FC
    .db E0
    .db BD
    .db 03
    .db 04
    .db F0
    .db BF
    .db 03
    .db F4
    .db F0
    .db C1
    .db 03
    .db FC
    .db F0
    .db C3
    .db 03
    .db 04
    .db 08
    .db D8
    .db C5
    .db 03
    .db E4
    .db E0
    .db C7
    .db 03
    .db EC
    .db E0
    .db C9
    .db 03
    .db F4
    .db E0
    .db CB
    .db 03
    .db FC
    .db E0
    .db BD
    .db 03
    .db 04
    .db F0
    .db CD
    .db 03
    .db F4
    .db F0
    .db CF
    .db 03
    .db FC
    .db F0
    .db C3
    .db 03
    .db 04
    .db 02
    .db F8
    .db ED
    .db 03
    .db F8
    .db F8
    .db EF
    .db 03
    .db 00
    .db 09
    .db D0
    .db 9D
    .db 03
    .db EF
    .db D0
    .db 9F
    .db 03
    .db F7
    .db D0
    .db 85
    .db 03
    .db FF
    .db E0
    .db A1
    .db 03
    .db F1
    .db E0
    .db 89
    .db 03
    .db F9
    .db E0
    .db 8B
    .db 03
    .db 01
    .db F0
    .db A3
    .db 03
    .db F4
    .db F0
    .db 8F
    .db 03
    .db FC
    .db F0
    .db 91
    .db 03
    .db 04
    .db 0C
    .db D0
    .db A5
    .db 03
    .db D8
    .db D0
    .db A7
    .db 03
    .db E0
    .db D0
    .db A9
    .db 03
    .db E8
    .db D0
    .db AB
    .db 03
    .db F0
    .db D0
    .db AD
    .db 03
    .db F8
    .db D0
    .db AF
    .db 03
    .db 00
    .db E0
    .db A1
    .db 03
    .db F1
    .db E0
    .db B1
    .db 03
    .db F9
    .db E0
    .db B3
    .db 03
    .db 01
    .db F0
    .db A3
    .db 03
    .db F4
    .db F0
    .db B5
    .db 03
    .db FC
    .db F0
    .db B7
    .db 03
    .db 04
    .db 05
    .db E0
    .db D1
    .db 03
    .db EF
    .db E0
    .db D3
    .db 03
    .db F7
    .db E0
    .db D5
    .db 03
    .db FF
    .db F0
    .db D7
    .db 03
    .db F8
    .db F0
    .db D9
    .db 03
    .db 00
    .db 05
    .db ED
    .db DF
    .db 03
    .db F0
    .db ED
    .db E1
    .db 03
    .db F8
    .db EE
    .db E3
    .db 03
    .db 00
    .db F0
    .db E5
    .db 03
    .db 08
    .db F0
    .db E7
    .db 03
    .db 10
    .db 03
    .db D0
    .db 99
    .db 02
    .db FC
    .db E0
    .db 95
    .db 03
    .db FC
    .db F0
    .db 97
    .db 03
    .db FC
    .db 03
    .db D0
    .db 9B
    .db 02
    .db FC
    .db E0
    .db 95
    .db 03
    .db FC
    .db F0
    .db 97
    .db 03
    .db FC
    .db 05
    .db D8
    .db A9
    .db 02
    .db 19
    .db E0
    .db A3
    .db 03
    .db 14
    .db E8
    .db A1
    .db 03
    .db 0C
    .db F0
    .db 9F
    .db 03
    .db 04
    .db F0
    .db 9D
    .db 03
    .db FC
    .db 05
    .db D8
    .db AB
    .db 02
    .db 19
    .db E0
    .db A3
    .db 03
    .db 14
    .db E8
    .db A1
    .db 03
    .db 0C
    .db F0
    .db 9F
    .db 03
    .db 04
    .db F0
    .db 9D
    .db 03
    .db FC
    .db 04
    .db C0
    .db E9
    .db 03
    .db FC
    .db D0
    .db EB
    .db 03
    .db FC
    .db E0
    .db EB
    .db 03
    .db FC
    .db F0
    .db ED
    .db 03
    .db FC
    .db 03
    .db D0
    .db E9
    .db 03
    .db FC
    .db E0
    .db EB
    .db 03
    .db FC
    .db F0
    .db ED
    .db 03
    .db FC
    .db 02
    .db E0
    .db E9
    .db 03
    .db FC
    .db F0
    .db ED
    .db 03
    .db FC
    .db 01
    .db F0
    .db EF
    .db 03
    .db FC
    .db 01
    .db F0
    .db F1
    .db 03
    .db FC
    .db 09
    .db D0
    .db 81
    .db 03
    .db F8
    .db D0
    .db 83
    .db 03
    .db 00
    .db E0
    .db 85
    .db 03
    .db F4
    .db E0
    .db 87
    .db 03
    .db FC
    .db E0
    .db 89
    .db 03
    .db 04
    .db F0
    .db 8B
    .db 03
    .db F0
    .db F0
    .db 8D
    .db 03
    .db F8
    .db F0
    .db 8F
    .db 03
    .db 00
    .db F0
    .db 91
    .db 03
    .db 08
    .db 0A
    .db D4
    .db 81
    .db 03
    .db F8
    .db D4
    .db 83
    .db 03
    .db 00
    .db E4
    .db 93
    .db 03
    .db F0
    .db E4
    .db 95
    .db 03
    .db F8
    .db E4
    .db 97
    .db 03
    .db 00
    .db E4
    .db 99
    .db 03
    .db 08
    .db F4
    .db 9B
    .db 03
    .db F0
    .db F4
    .db 9D
    .db 03
    .db F8
    .db F4
    .db 9F
    .db 03
    .db 00
    .db F4
    .db A1
    .db 03
    .db 08
    .db 09
    .db D0
    .db 81
    .db 03
    .db F8
    .db D0
    .db 83
    .db 03
    .db 00
    .db E0
    .db B7
    .db 03
    .db F4
    .db E0
    .db 87
    .db 03
    .db FC
    .db E0
    .db 89
    .db 03
    .db 04
    .db F0
    .db 8B
    .db 03
    .db F0
    .db F0
    .db 8D
    .db 03
    .db F8
    .db F0
    .db 8F
    .db 03
    .db 00
    .db F0
    .db 91
    .db 03
    .db 08
    .db 0A
    .db D4
    .db 81
    .db 03
    .db F8
    .db D4
    .db 83
    .db 03
    .db 00
    .db E4
    .db D9
    .db 03
    .db F0
    .db E4
    .db DB
    .db 03
    .db F8
    .db E4
    .db 97
    .db 03
    .db 00
    .db E4
    .db 99
    .db 03
    .db 08
    .db F4
    .db 9B
    .db 03
    .db F0
    .db F4
    .db 9D
    .db 03
    .db F8
    .db F4
    .db 9F
    .db 03
    .db 00
    .db F4
    .db A1
    .db 03
    .db 08
    .db 06
    .db D0
    .db A3
    .db 03
    .db F6
    .db D0
    .db A5
    .db 03
    .db FE
    .db E0
    .db A7
    .db 03
    .db F5
    .db E0
    .db A9
    .db 03
    .db FD
    .db F0
    .db AB
    .db 03
    .db F8
    .db F0
    .db AD
    .db 03
    .db 00
    .db 06
    .db D4
    .db A3
    .db 03
    .db F6
    .db D4
    .db A5
    .db 03
    .db FE
    .db E4
    .db AF
    .db 03
    .db F5
    .db E4
    .db B1
    .db 03
    .db FD
    .db F4
    .db B3
    .db 03
    .db F8
    .db F4
    .db B5
    .db 03
    .db 00
    .db 06
    .db D0
    .db A3
    .db 03
    .db F6
    .db D0
    .db A5
    .db 03
    .db FE
    .db E0
    .db B9
    .db 03
    .db F5
    .db E0
    .db A9
    .db 03
    .db FD
    .db F0
    .db AB
    .db 03
    .db F8
    .db F0
    .db AD
    .db 03
    .db 00
    .db 06
    .db D4
    .db A3
    .db 03
    .db F6
    .db D4
    .db A5
    .db 03
    .db FE
    .db E4
    .db DD
    .db 03
    .db F5
    .db E4
    .db B1
    .db 03
    .db FD
    .db F4
    .db B3
    .db 03
    .db F8
    .db F4
    .db B5
    .db 03
    .db 00
    .db 0C
    .db D9
    .db D3
    .db 03
    .db F4
    .db DC
    .db D5
    .db 03
    .db FC
    .db DB
    .db D7
    .db 03
    .db 04
    .db D0
    .db A3
    .db 03
    .db F6
    .db D0
    .db A5
    .db 03
    .db FE
    .db E0
    .db 85
    .db 03
    .db F4
    .db E0
    .db 87
    .db 03
    .db FC
    .db E0
    .db 89
    .db 03
    .db 04
    .db F0
    .db 8B
    .db 03
    .db F8
    .db F0
    .db 8D
    .db 03
    .db 00
    .db F0
    .db 8F
    .db 03
    .db 08
    .db F0
    .db 91
    .db 03
    .db 10
    .db 06
    .db E0
    .db BB
    .db 03
    .db F8
    .db E0
    .db BD
    .db 03
    .db 00
    .db E0
    .db BF
    .db 03
    .db 08
    .db F0
    .db C1
    .db 03
    .db F6
    .db F0
    .db C3
    .db 03
    .db FE
    .db F0
    .db C5
    .db 03
    .db 06
    .db 04
    .db F0
    .db CB
    .db 03
    .db F8
    .db F0
    .db CD
    .db 03
    .db 00
    .db F0
    .db CF
    .db 03
    .db 08
    .db F0
    .db D1
    .db 03
    .db 10
    .db 02
    .db F8
    .db E1
    .db 03
    .db F8
    .db F8
    .db E3
    .db 03
    .db 00
    .db 03
    .db F8
    .db E1
    .db 03
    .db F8
    .db F8
    .db E3
    .db 03
    .db 00
    .db F8
    .db E5
    .db 02
    .db 08
    .db 13
    .db 70
    .db 93
    .db 02
    .db FC
    .db 80
    .db 83
    .db 02
    .db F8
    .db 80
    .db 83
    .db 42
    .db 00
    .db 90
    .db 81
    .db 02
    .db F8
    .db 90
    .db 81
    .db 42
    .db 00
    .db A0
    .db 83
    .db 02
    .db F8
    .db A0
    .db 83
    .db 42
    .db 00
    .db B0
    .db 81
    .db 02
    .db F8
    .db B0
    .db 81
    .db 42
    .db 00
    .db C0
    .db 83
    .db 02
    .db F8
    .db C0
    .db 83
    .db 42
    .db 00
    .db D8
    .db 85
    .db 02
    .db F0
    .db D0
    .db 87
    .db 02
    .db F8
    .db D0
    .db 89
    .db 02
    .db 00
    .db DA
    .db 8B
    .db 02
    .db 08
    .db E0
    .db 8F
    .db 02
    .db F8
    .db E0
    .db 91
    .db 02
    .db 00
    .db F8
    .db F3
    .db 03
    .db F8
    .db F8
    .db F3
    .db 43
    .db 00
    .db 08
    .db E0
    .db B7
    .db 43
    .db F0
    .db E0
    .db B1
    .db 03
    .db F8
    .db E0
    .db B5
    .db 03
    .db 00
    .db E0
    .db B7
    .db 03
    .db 08
    .db F0
    .db B9
    .db 03
    .db F0
    .db F0
    .db BB
    .db 03
    .db F8
    .db F0
    .db BD
    .db 03
    .db 00
    .db F0
    .db B9
    .db 43
    .db 08
    .db 08
    .db E0
    .db A3
    .db 03
    .db F0
    .db E0
    .db A5
    .db 03
    .db F8
    .db E0
    .db A9
    .db C3
    .db 00
    .db E0
    .db A7
    .db C3
    .db 08
    .db F0
    .db A7
    .db 03
    .db F0
    .db F0
    .db A9
    .db 03
    .db F8
    .db F0
    .db A5
    .db C3
    .db 00
    .db F0
    .db A3
    .db C3
    .db 08
    .db 12
    .db D0
    .db C1
    .db 03
    .db E9
    .db D0
    .db C3
    .db 03
    .db F1
    .db D0
    .db C5
    .db 03
    .db F9
    .db D0
    .db C7
    .db 03
    .db 01
    .db D0
    .db C9
    .db 03
    .db 09
    .db E0
    .db CB
    .db 03
    .db E8
    .db E0
    .db CD
    .db 03
    .db F0
    .db E0
    .db CF
    .db 03
    .db F8
    .db E0
    .db D1
    .db 03
    .db 00
    .db E0
    .db D3
    .db 03
    .db 08
    .db E0
    .db D5
    .db 03
    .db 10
    .db F0
    .db D7
    .db 03
    .db E0
    .db F0
    .db D9
    .db 03
    .db E8
    .db F0
    .db DB
    .db 03
    .db F0
    .db F0
    .db DD
    .db 03
    .db F8
    .db F0
    .db DF
    .db 03
    .db 00
    .db F0
    .db E1
    .db 03
    .db 08
    .db F0
    .db E3
    .db 03
    .db 10
    .db 11
    .db D0
    .db 8F
    .db 03
    .db 10
    .db D2
    .db 91
    .db 03
    .db 18
    .db CE
    .db 93
    .db 03
    .db 20
    .db CE
    .db 95
    .db 03
    .db 28
    .db E3
    .db 81
    .db 03
    .db E0
    .db E3
    .db 83
    .db 03
    .db E8
    .db E0
    .db 85
    .db 03
    .db F0
    .db E0
    .db 87
    .db 03
    .db F8
    .db E0
    .db 89
    .db 03
    .db 00
    .db E0
    .db 8B
    .db 03
    .db 08
    .db E0
    .db 8D
    .db 03
    .db 10
    .db F3
    .db 97
    .db 03
    .db E8
    .db F0
    .db 99
    .db 03
    .db F0
    .db F0
    .db 9B
    .db 03
    .db F8
    .db F0
    .db 9D
    .db 03
    .db 00
    .db F0
    .db 9F
    .db 03
    .db 08
    .db F0
    .db A1
    .db 03
    .db 10
    .db 12
    .db D0
    .db 93
    .db 03
    .db 20
    .db D0
    .db 95
    .db 03
    .db 28
    .db E0
    .db 81
    .db 03
    .db E0
    .db E0
    .db 83
    .db 03
    .db E8
    .db E0
    .db A3
    .db 03
    .db F0
    .db E0
    .db A5
    .db 03
    .db F8
    .db E0
    .db A7
    .db 03
    .db 00
    .db E0
    .db A9
    .db 03
    .db 08
    .db E0
    .db AB
    .db 03
    .db 10
    .db DD
    .db AD
    .db 03
    .db 18
    .db F0
    .db B1
    .db 03
    .db E0
    .db F0
    .db B3
    .db 03
    .db E8
    .db F0
    .db B5
    .db 03
    .db F0
    .db F0
    .db B7
    .db 03
    .db F8
    .db F0
    .db B9
    .db 03
    .db 00
    .db F0
    .db BB
    .db 03
    .db 08
    .db F0
    .db BD
    .db 03
    .db 10
    .db ED
    .db AF
    .db 03
    .db 18
    .db 11
    .db D0
    .db D5
    .db 03
    .db 10
    .db D0
    .db D7
    .db 03
    .db 18
    .db D0
    .db D9
    .db 03
    .db 20
    .db D0
    .db DB
    .db 03
    .db 28
    .db E4
    .db 81
    .db 03
    .db E0
    .db E4
    .db 83
    .db 03
    .db E8
    .db E0
    .db C1
    .db 03
    .db F0
    .db E0
    .db C3
    .db 03
    .db F8
    .db E0
    .db C5
    .db 03
    .db 00
    .db E0
    .db C7
    .db 03
    .db 08
    .db E0
    .db C9
    .db 03
    .db 10
    .db F4
    .db BF
    .db 03
    .db E8
    .db F0
    .db CB
    .db 03
    .db F0
    .db F0
    .db CD
    .db 03
    .db F8
    .db F0
    .db CF
    .db 03
    .db 00
    .db F0
    .db D1
    .db 03
    .db 08
    .db F0
    .db D3
    .db 03
    .db 10
    .db 10
    .db CA
    .db F7
    .db 03
    .db 20
    .db D0
    .db F3
    .db 03
    .db 10
    .db D0
    .db F5
    .db 03
    .db 18
    .db E4
    .db 81
    .db 03
    .db E0
    .db E4
    .db 83
    .db 03
    .db E8
    .db E0
    .db DF
    .db 03
    .db F0
    .db E0
    .db E1
    .db 03
    .db F8
    .db E0
    .db E3
    .db 03
    .db 00
    .db E0
    .db E5
    .db 03
    .db 08
    .db E0
    .db E7
    .db 03
    .db 10
    .db F4
    .db DD
    .db 03
    .db E8
    .db F0
    .db E9
    .db 03
    .db F0
    .db F0
    .db EB
    .db 03
    .db F8
    .db F0
    .db ED
    .db 03
    .db 00
    .db F0
    .db EF
    .db 03
    .db 08
    .db F0
    .db F1
    .db 03
    .db 10
    .db 12
    .db D0
    .db 93
    .db 03
    .db 20
    .db D0
    .db 95
    .db 03
    .db 28
    .db E0
    .db 81
    .db 03
    .db E0
    .db E0
    .db 83
    .db 03
    .db E8
    .db E0
    .db A3
    .db 03
    .db F0
    .db E0
    .db A5
    .db 03
    .db F8
    .db E0
    .db A7
    .db 03
    .db 00
    .db E0
    .db A9
    .db 03
    .db 08
    .db E0
    .db AB
    .db 03
    .db 10
    .db DD
    .db AD
    .db 03
    .db 18
    .db F0
    .db B1
    .db 03
    .db E0
    .db F0
    .db E5
    .db 03
    .db E8
    .db F0
    .db E7
    .db 03
    .db F0
    .db F0
    .db E9
    .db 03
    .db F8
    .db F0
    .db EB
    .db 03
    .db 00
    .db F0
    .db ED
    .db 03
    .db 08
    .db F0
    .db EF
    .db 03
    .db 10
    .db ED
    .db AF
    .db 03
    .db 18
    .db 11
    .db D0
    .db 8F
    .db 03
    .db 10
    .db D2
    .db 91
    .db 03
    .db 18
    .db CE
    .db 93
    .db 03
    .db 20
    .db CE
    .db 95
    .db 03
    .db 28
    .db E3
    .db 81
    .db 03
    .db E0
    .db E3
    .db 83
    .db 03
    .db E8
    .db E0
    .db 85
    .db 03
    .db F0
    .db E0
    .db 87
    .db 03
    .db F8
    .db E0
    .db 89
    .db 03
    .db 00
    .db E0
    .db 8B
    .db 03
    .db 08
    .db E0
    .db 8D
    .db 03
    .db 10
    .db F3
    .db F5
    .db 03
    .db E8
    .db F0
    .db F7
    .db 03
    .db F0
    .db F0
    .db F9
    .db 03
    .db F8
    .db F0
    .db FB
    .db 03
    .db 00
    .db F0
    .db FD
    .db 03
    .db 08
    .db F0
    .db FF
    .db 03
    .db 10
    .db 08
    .db F0
    .db F1
    .db 03
    .db E0
    .db F0
    .db F3
    .db 03
    .db E8
    .db F0
    .db F3
    .db 03
    .db F0
    .db F0
    .db F3
    .db 03
    .db F8
    .db F0
    .db F3
    .db 03
    .db 00
    .db F0
    .db F3
    .db 03
    .db 08
    .db F0
    .db F3
    .db 03
    .db 10
    .db F0
    .db F1
    .db 43
    .db 18
    .db 01
    .db F8
    .db F9
    .db 02
    .db FC
    .db 01
    .db F8
    .db FB
    .db 02
    .db FC
    .db 01
    .db F8
    .db FD
    .db 02
    .db FC
    .db 0B
    .db E5
    .db D3
    .db 03
    .db F4
    .db E8
    .db D5
    .db 03
    .db FC
    .db E7
    .db D7
    .db 03
    .db 04
    .db D0
    .db A3
    .db 03
    .db F6
    .db D0
    .db A5
    .db 03
    .db FE
    .db E0
    .db A7
    .db 03
    .db F5
    .db E0
    .db A9
    .db 03
    .db FD
    .db F0
    .db 8B
    .db 03
    .db F8
    .db F0
    .db 8D
    .db 03
    .db 00
    .db F0
    .db 8F
    .db 03
    .db 08
    .db F0
    .db 91
    .db 03
    .db 10
    .db 09
    .db ED
    .db D3
    .db 03
    .db F4
    .db F0
    .db D5
    .db 03
    .db FC
    .db DB
    .db D7
    .db 03
    .db 04
    .db D0
    .db A3
    .db 03
    .db F6
    .db D0
    .db A5
    .db 03
    .db FE
    .db E0
    .db A7
    .db 03
    .db F5
    .db E0
    .db A9
    .db 03
    .db FD
    .db F0
    .db AB
    .db 03
    .db F8
    .db F0
    .db AD
    .db 03
    .db 00
    .db 02
    .db F8
    .db E7
    .db 03
    .db F8
    .db F8
    .db E9
    .db 03
    .db 00
    .db 03
    .db F8
    .db E7
    .db 03
    .db F8
    .db F8
    .db E9
    .db 03
    .db 00
    .db 00
    .db EB
    .db 02
    .db 01
    .db 02
    .db F8
    .db ED
    .db 03
    .db F8
    .db F8
    .db EF
    .db 03
    .db 00
    .db 03
    .db F8
    .db ED
    .db 03
    .db F8
    .db F8
    .db EF
    .db 03
    .db 00
    .db 08
    .db F1
    .db 02
    .db FD
    .db 06
    .db E0
    .db C7
    .db 03
    .db F8
    .db E0
    .db C9
    .db 03
    .db 00
    .db E0
    .db BF
    .db 03
    .db 08
    .db F0
    .db C1
    .db 03
    .db F6
    .db F0
    .db C3
    .db 03
    .db FE
    .db F0
    .db C5
    .db 03
    .db 06
    .db 0C
    .db E0
    .db C5
    .db 02
    .db E8
    .db E0
    .db C7
    .db 02
    .db F0
    .db E0
    .db C9
    .db 02
    .db F8
    .db E0
    .db CB
    .db 02
    .db 00
    .db E0
    .db CD
    .db 02
    .db 08
    .db E0
    .db CF
    .db 02
    .db 10
    .db F0
    .db B9
    .db 43
    .db EC
    .db F0
    .db FD
    .db 43
    .db F4
    .db F0
    .db B9
    .db 43
    .db 00
    .db F0
    .db FD
    .db 43
    .db 08
    .db F0
    .db B9
    .db 43
    .db F8
    .db F0
    .db FD
    .db 43
    .db 00
    .db 0C
    .db DF
    .db D5
    .db 02
    .db E8
    .db DF
    .db D7
    .db 02
    .db F0
    .db DF
    .db C9
    .db 02
    .db F8
    .db DF
    .db D9
    .db 02
    .db 00
    .db DF
    .db CD
    .db 02
    .db 08
    .db DF
    .db CF
    .db 02
    .db 10
    .db EF
    .db BD
    .db 43
    .db EC
    .db EF
    .db BB
    .db 43
    .db F4
    .db EF
    .db BD
    .db 43
    .db 00
    .db EF
    .db BB
    .db 43
    .db 08
    .db EF
    .db BD
    .db 43
    .db F8
    .db EF
    .db BB
    .db 43
    .db 00
    .db 01
    .db F8
    .db F1
    .db 03
    .db FC
    .db 02
    .db F8
    .db E9
    .db 03
    .db F8
    .db F8
    .db EB
    .db 03
    .db 00
    .db 0C
    .db DC
    .db 81
    .db 03
    .db D8
    .db DC
    .db 83
    .db 03
    .db E0
    .db DD
    .db 83
    .db 03
    .db EA
    .db DE
    .db 83
    .db 03
    .db F4
    .db E0
    .db 83
    .db 03
    .db FE
    .db E0
    .db C1
    .db 03
    .db 08
    .db F0
    .db 89
    .db 03
    .db E8
    .db F0
    .db 8B
    .db 03
    .db F0
    .db F0
    .db 8D
    .db 03
    .db F8
    .db F0
    .db 8F
    .db 03
    .db 00
    .db F0
    .db BD
    .db 03
    .db 08
    .db F0
    .db BF
    .db 03
    .db 10
    .db 08
    .db E0
    .db AB
    .db 03
    .db F0
    .db E0
    .db AD
    .db 03
    .db F8
    .db E0
    .db BF
    .db C3
    .db 00
    .db E0
    .db AF
    .db C3
    .db 08
    .db F0
    .db AF
    .db 03
    .db F0
    .db F0
    .db BF
    .db 03
    .db F8
    .db F0
    .db AD
    .db C3
    .db 00
    .db F0
    .db AB
    .db C3
    .db 08
SCREEN_0x07: ; 09:089D, 0x01289D
    JSR SUB_CHECK_OBJS-0x7-0x11_RET_CS_FINISHED ; Check objects.
    BCC RTS ; Not finished.
    LDA 60C_UNK_INDEX
    CMP #$03
    BCS RTS
    LDY 7E_STREAM_UNK
    LDA L_09:08C8,Y
    STA 60C_UNK_INDEX
    CMP #$03
    BEQ RTS
    LDA 7E_STREAM_UNK
    ASL A
    TAY
    LDA L_09:0FCE,Y
    STA TMP_00
    LDA L_09:0FCF,Y
    STA TMP_01
    LDY #$FF
    JMP L_09:090B
L_09:08C8: ; 09:08C8, 0x0128C8
    .db 00
    .db 00
    .db 01
    .db 01
    .db 02
    .db 02
    .db 03
RTN_OBJECTS_SPAWN?: ; 09:08CF, 0x0128CF
    LDA LEVEL/SCREEN_ON ; Load
    CMP #$07 ; If _ #$07
    BEQ SCREEN_0x07 ; ==, goto.
    ASL A ; << 1, *2
    TAY ; To Y index.
    LDA DATA_PTR_UNK_L,Y ; Set up file.
    STA TMP_10
    LDA DATA_PTR_UNK_H,Y
    STA TMP_11
    LDA 7E_STREAM_UNK ; Load val.
    ASL A ; << 1, *2
    TAY ; To Y index.
    LDA [TMP_10],Y ; Load from stream.
    STA TMP_00 ; Store to.
    INY ; Stream++
    LDA [TMP_10],Y ; Load from stream.
    STA TMP_01 ; Store to.
    LDY #$00 ; Stream reset.
    LDA [TMP_00],Y ; Load from other stream.
    BMI RTS ; If negative, leave.
    STA TMP_12 ; Store val from stream.
    AND #$1F ; Keep 0001.1111
    CMP SCRIPT_LEVEL_SCREEN[2] ; If _ var
    BNE COMPARE_LEAVE ; !=, goto.
    INY ; Stream++
    LDA [TMP_00],Y ; Load from stream.
    CMP SCRIPT_SCREEN_X_SCROLL[2] ; If A _ var
    BEQ DONT_LEAVE ; ==, goto.
    BCS RTS ; >=, leave.
DONT_LEAVE: ; 09:0905, 0x012905
    LDA TMP_12 ; Load 
    AND #$20 ; Keep 0010.0000
    BNE OBJECT_4_START? ; Set, goto.
L_09:090B: ; 09:090B, 0x01290B
    LDX #$07 ; Object.
LOOP_OBJS_4: ; 09:090D, 0x01290D
    LDA OBJ_ENABLED_STATE+MORE?[18],X ; Load obj.
    BEQ OBJECT_AVAILABLE_7 ; If available, goto.
OBJ_7_PAIR_UNAVAILABLE: ; 09:0912, 0x012912
    INX ; Obj += 2
    INX
    CPX #$11 ; If _ #$11
    BCC LOOP_OBJS_4 ; <, goto.
    LDA TMP_12 ; Load
    AND #$40 ; Test 0100.0000
    BEQ RTS ; Not set, leave.
    INC BG_UPDATE_FLAG_UNK ; ++
RTS: ; 09:0920, 0x012920
    RTS
OBJECT_4_START?: ; 09:0921, 0x012921
    LDX #$04 ; Object.
LOOP_OBJS_7: ; 09:0923, 0x012923
    LDA OBJ_ENABLED_STATE+MORE?[18],X ; Load
    BEQ OBJECT_AVAILABLE_4 ; Available, goto.
    INX ; Obj++
    CPX #$11 ; If obj _ #$11
    BCC LOOP_OBJS_7 ; <, goto.
    RTS
COMPARE_LEAVE: ; 09:092E, 0x01292E
    BCS RTS ; >=, RTS no inc.
    INC 7E_STREAM_UNK ; ++
    RTS ; Leave.
OBJECT_AVAILABLE_7: ; 09:0933, 0x012933
    LDA OBJ_ENABLED_STATE+MORE?+1,X ; Load next object.
    BNE OBJ_7_PAIR_UNAVAILABLE ; Not available, re-enter.
OBJECT_AVAILABLE_4: ; 09:0938, 0x012938
    LDA TMP_12 ; Load
    AND #$40 ; Test bit.
    BEQ DONT_INC_VARS ; Not set, goto.
    INC BG_UPDATE_FLAG_UNK ; ++
    INC 5FA_UNK ; ++
DONT_INC_VARS: ; 09:0943, 0x012943
    INY ; Stream++
    LDA [TMP_00],Y ; Load from stream.
    STA TMP_13 ; Store to.
    AND #$40 ; Test 0100.0000
    BEQ ZP_13_0x40_UNSET ; Bit not set, goto.
    JSR TEST_OBJ[7-18]_DISABLED/!8B_RET_CS_TRUE ; Test 0x07-0x12
    BCC RTS ; Not all done, leave.
ZP_13_0x40_UNSET: ; 09:0951, 0x012951
    LDA TMP_13 ; Load
    AND #$30 ; Test 0011.0000
    BEQ ZP_13_0x30_UNSET ; None set, goto.
    LSR A ; Shift to  index.
    LSR A
    LSR A
    LSR A
    STA ZP_13_INDEX_VAL_UNK
    LDA TMP_13 ; Load
    BPL ZP_13_POSITIVE ; If positive, goto.
    INC 70C_END_OF_LEVEL_HOLDER? ; ++ var.
    LDA #$10
    STA 5FB_TIMER_ALL_FINISHED? ; Set.
ZP_13_POSITIVE: ; 09:096A, 0x01296A
    LDA TMP_13 ; Load
    AND #$0F ; Keep bottom nibble.
    STA 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Store to object.
    INY ; Stream++
    LDA [TMP_00],Y ; Load from stream.
    STA TMP_14 ; Store to.
    LSR A ; Shift upper nibble down.
    LSR A
    LSR A
    LSR A
    STA STATUS_FLAGS_B/OBJ_FOCUS[18],X ; Store to object.
    INY ; Stream++
    LDA [TMP_00],Y ; Load from, stream.
    STA 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X ; Store to object.
    LDA #$23 ; Which state?
    BNE SET_OBJECT_STATE/ENABLED ; Always taken.
ZP_13_0x30_UNSET: ; 09:0987, 0x012987
    LDA TMP_13 ; Load
    BPL ZP_13_POSITIVE ; If positive, goto.
    INC 70C_END_OF_LEVEL_HOLDER? ; ++
    LDA #$03
    STA 5FB_TIMER_ALL_FINISHED? ; Set.
    LDA TMP_13 ; Reload.
ZP_13_POSITIVE: ; 09:0995, 0x012995
    AND #$40 ; Test bit 0100.0000
    BEQ ZP_13_0x40_UNSET ; Not set, goto.
    INC 5F8_OBJ_SETS_UNK ; Inc if set.
ZP_13_0x40_UNSET: ; 09:099C, 0x01299C
    LDA TMP_13 ; Load
    AND #$0F ; Get bottom nibble.
    STA 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Store to object.
    INY ; Stream++
    LDA [TMP_00],Y ; Load from stream.
    STA TMP_14 ; Store to ZP_14
    AND #$03 ; Keep 0000.0011
    STA 556_OBJ_STATUS_FLAGS_A[18],X ; Store to object.
    INY ; Stream++
    LDA [TMP_00],Y ; Load from stream.
SET_OBJECT_STATE/ENABLED: ; 09:09B0, 0x0129B0
    STA OBJ_ENABLED_STATE+MORE?[18],X ; Store to object.
    LDA TMP_14 ; Load
    AND #$08 ; Keep 0000.1000
    LSR A ; >> 3, /8
    LSR A
    LSR A
    STA 712_FLAG_PALETTE_UNK ; Store to.
    LDA TMP_14 ; Load
    AND #$04 ; Keep 0000.0100
    LSR A ; >> 2, /4
    LSR A
    STA DE_FLAG_PALETTE_UNK
    INY ; Stream++
    LDA [TMP_00],Y ; Load from stream.
    STA OBJ_POS_X_CONFIRMED[18],X ; Store to object.
    INY ; Stream++
    LDA [TMP_00],Y ; Load from stream.
    STA OBJ_POS_X??[18],X ; Store to obj. Y pos, guessing?
    INC 7E_STREAM_UNK ; ++
    RTS ; Leave.
DATA_PTR_UNK_L: ; 09:09D4, 0x0129D4
    .db EC
DATA_PTR_UNK_H: ; 09:09D5, 0x0129D5
    .db A9
    .db 22
    .db AA
    .db 00
    .db AB
    .db 1A
    .db AC
    .db 73
    .db B3
    .db 07
    .db AD
    .db 75
    .db AE
    .db CE
    .db AF
    .db 03
    .db B0
    .db 72
    .db B4
    .db 6C
    .db B1
    .db DC
    .db B1
    .db 2C
    .db AA
    .db 33
    .db AA
    .db 3A
    .db AA
    .db 41
    .db AA
    .db 48
    .db AA
    .db 4F
    .db AA
    .db 56
    .db AA
    .db 5D
    .db AA
    .db 64
    .db AA
    .db 6B
    .db AA
    .db 72
    .db AA
    .db 79
    .db AA
    .db 80
    .db AA
    .db 87
    .db AA
    .db 8E
    .db AA
    .db 95
    .db AA
    .db 9C
    .db AA
    .db A3
    .db AA
    .db AA
    .db AA
    .db B1
    .db AA
    .db B8
    .db AA
    .db BF
    .db AA
    .db C6
    .db AA
    .db CD
    .db AA
    .db D4
    .db AA
    .db DB
    .db AA
    .db E2
    .db AA
    .db E3
    .db AA
    .db EA
    .db AA
    .db F1
    .db AA
    .db F8
    .db AA
    .db FF
    .db AA
    .db 00
    .db 00
    .db 00
    .db 02
    .db 02
    .db E0
    .db 90
    .db 40
    .db 40
    .db 10
    .db 40
    .db 02
    .db 28
    .db C0
    .db 40
    .db 80
    .db 10
    .db 40
    .db 05
    .db 20
    .db 90
    .db 01
    .db 00
    .db 00
    .db 00
    .db B0
    .db B0
    .db 80
    .db 01
    .db 00
    .db 10
    .db 20
    .db 02
    .db E0
    .db 80
    .db 41
    .db 40
    .db 10
    .db 40
    .db 02
    .db E0
    .db 80
    .db 01
    .db 60
    .db 00
    .db 08
    .db 80
    .db B0
    .db 50
    .db 01
    .db 70
    .db 10
    .db 2C
    .db 03
    .db E0
    .db 80
    .db 41
    .db D0
    .db 10
    .db 48
    .db 03
    .db E0
    .db 80
    .db 02
    .db 80
    .db 00
    .db 00
    .db B0
    .db B0
    .db 80
    .db 42
    .db 80
    .db 10
    .db 50
    .db 02
    .db B0
    .db 80
    .db 42
    .db C0
    .db 10
    .db 60
    .db 05
    .db E0
    .db 80
    .db 03
    .db 60
    .db 00
    .db 08
    .db 80
    .db B0
    .db 50
    .db 03
    .db 70
    .db 10
    .db 2C
    .db 03
    .db E0
    .db 80
    .db 43
    .db D0
    .db 10
    .db 48
    .db 03
    .db E0
    .db 80
    .db 04
    .db 60
    .db 00
    .db 00
    .db B0
    .db 50
    .db 80
    .db 44
    .db 60
    .db 10
    .db 40
    .db 02
    .db E0
    .db 80
    .db 04
    .db 80
    .db 00
    .db 00
    .db B0
    .db B0
    .db 80
    .db 44
    .db 80
    .db 10
    .db 30
    .db 02
    .db E0
    .db 80
    .db 04
    .db C0
    .db 10
    .db 30
    .db 06
    .db E0
    .db 80
    .db 44
    .db E0
    .db 10
    .db 40
    .db 06
    .db E0
    .db 80
    .db 05
    .db 70
    .db 00
    .db 00
    .db B1
    .db 40
    .db 7D
    .db 45
    .db 80
    .db 10
    .db 40
    .db 02
    .db E0
    .db 80
    .db 06
    .db 00
    .db 00
    .db 00
    .db 32
    .db D8
    .db A0
    .db 06
    .db 00
    .db 00
    .db 00
    .db 32
    .db E0
    .db B8
    .db 06
    .db 00
    .db 80
    .db 00
    .db 32
    .db E8
    .db D0
    .db FF
    .db 00
    .db 00
    .db 00
    .db 01
    .db 06
    .db 20
    .db B0
    .db 00
    .db 00
    .db 00
    .db 01
    .db 06
    .db 18
    .db C8
    .db 40
    .db 60
    .db 10
    .db 20
    .db 04
    .db A9
    .db 8F
    .db 00
    .db 80
    .db C0
    .db 00
    .db 51
    .db A8
    .db 8F
    .db FF
    .db 40
    .db AB
    .db 47
    .db AB
    .db 4E
    .db AB
    .db 55
    .db AB
    .db 5C
    .db AB
    .db 63
    .db AB
    .db 6A
    .db AB
    .db 71
    .db AB
    .db 78
    .db AB
    .db 7F
    .db AB
    .db 86
    .db AB
    .db 8D
    .db AB
    .db 94
    .db AB
    .db 9B
    .db AB
    .db A2
    .db AB
    .db A9
    .db AB
    .db B0
    .db AB
    .db B7
    .db AB
    .db BE
    .db AB
    .db C5
    .db AB
    .db CC
    .db AB
    .db D3
    .db AB
    .db DA
    .db AB
    .db E1
    .db AB
    .db E8
    .db AB
    .db EF
    .db AB
    .db F6
    .db AB
    .db FD
    .db AB
    .db 04
    .db AC
    .db 0B
    .db AC
    .db 12
    .db AC
    .db 19
    .db AC
    .db 00
    .db 01
    .db 07
    .db 00
    .db B2
    .db 30
    .db C0
    .db 40
    .db 20
    .db 10
    .db 40
    .db 02
    .db 30
    .db C0
    .db 00
    .db 30
    .db 00
    .db 00
    .db B5
    .db A4
    .db A0
    .db 40
    .db 40
    .db 10
    .db 40
    .db 02
    .db 30
    .db C0
    .db 00
    .db C5
    .db 00
    .db 09
    .db 83
    .db 20
    .db D5
    .db 00
    .db E0
    .db 10
    .db 38
    .db 02
    .db 30
    .db C0
    .db 01
    .db 00
    .db 00
    .db 08
    .db 82
    .db 80
    .db 80
    .db 41
    .db 60
    .db 10
    .db 48
    .db 02
    .db 30
    .db C0
    .db 02
    .db 00
    .db 07
    .db 00
    .db B2
    .db E0
    .db 90
    .db 42
    .db 80
    .db 10
    .db 40
    .db 02
    .db 30
    .db C0
    .db 43
    .db 00
    .db 10
    .db 60
    .db 0B
    .db 30
    .db C0
    .db 03
    .db D8
    .db 00
    .db 01
    .db 8B
    .db 20
    .db D0
    .db 04
    .db 48
    .db 00
    .db 01
    .db 02
    .db 20
    .db D0
    .db 04
    .db 48
    .db 00
    .db 02
    .db 02
    .db E0
    .db D0
    .db 44
    .db 50
    .db 11
    .db 41
    .db 02
    .db 20
    .db CA
    .db 04
    .db 80
    .db 00
    .db 01
    .db 8C
    .db 20
    .db D0
    .db 05
    .db 48
    .db 00
    .db 00
    .db B3
    .db EA
    .db D0
    .db 45
    .db 50
    .db 30
    .db 20
    .db 02
    .db 30
    .db C0
    .db 05
    .db 70
    .db 00
    .db 00
    .db B3
    .db EA
    .db D8
    .db 45
    .db 78
    .db 30
    .db 20
    .db 07
    .db 30
    .db C0
    .db 05
    .db A8
    .db 00
    .db 00
    .db B3
    .db EA
    .db D0
    .db 45
    .db B8
    .db 30
    .db 20
    .db 02
    .db 30
    .db C0
    .db 05
    .db C8
    .db 00
    .db 00
    .db B3
    .db EA
    .db D8
    .db 45
    .db D0
    .db 30
    .db 20
    .db 07
    .db 30
    .db C0
    .db 07
    .db 00
    .db 00
    .db 00
    .db B7
    .db 70
    .db B0
    .db 07
    .db 00
    .db 00
    .db 00
    .db B8
    .db D0
    .db B0
    .db 47
    .db 00
    .db 10
    .db 40
    .db 07
    .db 30
    .db C0
    .db 48
    .db 00
    .db 10
    .db 60
    .db 02
    .db 30
    .db C0
    .db 09
    .db 60
    .db 07
    .db 00
    .db B2
    .db D0
    .db D0
    .db 49
    .db 70
    .db 10
    .db 30
    .db 07
    .db C0
    .db C0
    .db 09
    .db 80
    .db 80
    .db 00
    .db 53
    .db 30
    .db 84
    .db FF
    .db 50
    .db AC
    .db 57
    .db AC
    .db 5E
    .db AC
    .db 65
    .db AC
    .db 6C
    .db AC
    .db 73
    .db AC
    .db 7A
    .db AC
    .db 81
    .db AC
    .db 88
    .db AC
    .db 8F
    .db AC
    .db 96
    .db AC
    .db 9D
    .db AC
    .db A4
    .db AC
    .db AB
    .db AC
    .db B2
    .db AC
    .db B9
    .db AC
    .db C0
    .db AC
    .db C7
    .db AC
    .db CE
    .db AC
    .db D5
    .db AC
    .db DC
    .db AC
    .db E3
    .db AC
    .db EA
    .db AC
    .db F1
    .db AC
    .db F8
    .db AC
    .db FF
    .db AC
    .db 06
    .db AD
    .db 00
    .db 00
    .db 02
    .db 01
    .db 02
    .db 10
    .db CA
    .db 00
    .db 00
    .db 02
    .db 01
    .db 02
    .db 18
    .db E0
    .db 40
    .db 40
    .db 11
    .db 4C
    .db 03
    .db 18
    .db 90
    .db 40
    .db 80
    .db 11
    .db 40
    .db 08
    .db 18
    .db 90
    .db 00
    .db 90
    .db 00
    .db 00
    .db 85
    .db 34
    .db D0
    .db 00
    .db A1
    .db 01
    .db 09
    .db 02
    .db 20
    .db 90
    .db 00
    .db A1
    .db 01
    .db 0A
    .db 02
    .db E0
    .db 90
    .db 40
    .db C0
    .db 11
    .db 48
    .db 02
    .db 18
    .db 90
    .db 41
    .db 00
    .db 00
    .db 40
    .db 34
    .db D0
    .db 8F
    .db 41
    .db 80
    .db 00
    .db 40
    .db 34
    .db D0
    .db 8F
    .db 01
    .db C0
    .db 01
    .db 01
    .db 08
    .db 18
    .db 90
    .db 01
    .db C0
    .db 01
    .db 02
    .db 08
    .db E8
    .db 90
    .db 42
    .db 00
    .db 11
    .db 40
    .db 08
    .db 18
    .db 90
    .db 02
    .db 80
    .db 02
    .db 01
    .db 06
    .db 18
    .db D0
    .db 02
    .db 80
    .db 02
    .db 02
    .db 06
    .db E8
    .db E0
    .db 42
    .db B0
    .db 11
    .db 60
    .db 06
    .db 18
    .db 90
    .db 43
    .db 30
    .db 00
    .db 40
    .db 34
    .db A0
    .db 8F
    .db 03
    .db 38
    .db 00
    .db 00
    .db 85
    .db CC
    .db D0
    .db 03
    .db 40
    .db 00
    .db 00
    .db 8E
    .db D0
    .db 70
    .db 03
    .db 48
    .db 01
    .db 09
    .db 02
    .db 20
    .db D0
    .db 03
    .db 48
    .db 01
    .db 0A
    .db 02
    .db E0
    .db D0
    .db 44
    .db 00
    .db 11
    .db 48
    .db 02
    .db E0
    .db D0
    .db 44
    .db 60
    .db 11
    .db 50
    .db 06
    .db E0
    .db D0
    .db 44
    .db 80
    .db 00
    .db 40
    .db 34
    .db D0
    .db 8F
    .db 44
    .db B0
    .db 11
    .db 5C
    .db 03
    .db E0
    .db 90
    .db 05
    .db 00
    .db 80
    .db 00
    .db 55
    .db 20
    .db C0
    .db FF
    .db 55
    .db AD
    .db 5C
    .db AD
    .db 63
    .db AD
    .db 6A
    .db AD
    .db 71
    .db AD
    .db 78
    .db AD
    .db 7F
    .db AD
    .db 86
    .db AD
    .db 8D
    .db AD
    .db 94
    .db AD
    .db 9B
    .db AD
    .db A2
    .db AD
    .db BE
    .db AD
    .db C5
    .db AD
    .db CC
    .db AD
    .db D3
    .db AD
    .db DA
    .db AD
    .db E1
    .db AD
    .db E8
    .db AD
    .db EF
    .db AD
    .db F6
    .db AD
    .db FD
    .db AD
    .db 04
    .db AE
    .db 0B
    .db AE
    .db 12
    .db AE
    .db 19
    .db AE
    .db 20
    .db AE
    .db 27
    .db AE
    .db 2E
    .db AE
    .db 35
    .db AE
    .db 3C
    .db AE
    .db 43
    .db AE
    .db 4A
    .db AE
    .db 51
    .db AE
    .db 58
    .db AE
    .db 5F
    .db AE
    .db 66
    .db AE
    .db 6D
    .db AE
    .db 74
    .db AE
    .db 00
    .db 00
    .db 00
    .db 08
    .db 87
    .db E0
    .db A0
    .db 00
    .db 00
    .db 00
    .db 08
    .db 87
    .db EC
    .db B8
    .db 00
    .db 00
    .db 00
    .db 08
    .db 87
    .db F8
    .db D0
    .db 00
    .db 00
    .db 00
    .db 09
    .db 02
    .db 20
    .db A0
    .db 00
    .db 00
    .db 00
    .db 09
    .db 02
    .db 10
    .db D0
    .db 40
    .db 80
    .db 10
    .db 48
    .db 02
    .db 10
    .db A0
    .db 41
    .db 40
    .db 10
    .db 60
    .db 06
    .db 10
    .db A0
    .db 01
    .db 50
    .db 00
    .db 00
    .db 9E
    .db 80
    .db 80
    .db 01
    .db A0
    .db 00
    .db 09
    .db 02
    .db 20
    .db A0
    .db 01
    .db A0
    .db 00
    .db 0A
    .db 02
    .db E0
    .db C0
    .db 01
    .db C0
    .db 00
    .db 09
    .db 02
    .db 20
    .db B0
    .db 42
    .db 10
    .db 10
    .db 68
    .db 02
    .db 10
    .db A0
    .db 02
    .db 30
    .db 00
    .db 01
    .db 09
    .db 18
    .db A0
    .db 02
    .db 30
    .db 00
    .db 01
    .db 09
    .db 28
    .db C0
    .db 42
    .db 50
    .db 10
    .db 40
    .db 09
    .db 10
    .db A0
    .db 02
    .db A0
    .db 00
    .db 0D
    .db 03
    .db 18
    .db A0
    .db 02
    .db A0
    .db 00
    .db 09
    .db 03
    .db 28
    .db C0
    .db 02
    .db A0
    .db 00
    .db 0A
    .db 03
    .db E0
    .db B0
    .db 43
    .db 00
    .db 10
    .db 68
    .db 03
    .db 10
    .db A0
    .db 03
    .db 80
    .db 00
    .db 01
    .db 02
    .db 20
    .db B0
    .db 03
    .db 80
    .db 00
    .db 02
    .db 02
    .db E8
    .db A0
    .db 43
    .db E0
    .db 10
    .db 60
    .db 02
    .db 10
    .db A0
    .db 04
    .db FC
    .db 00
    .db 00
    .db 8D
    .db 20
    .db D0
    .db 04
    .db FC
    .db 04
    .db 00
    .db 24
    .db 48
    .db D0
    .db 45
    .db 10
    .db 10
    .db 30
    .db 04
    .db 48
    .db D0
    .db 05
    .db 20
    .db 00
    .db 00
    .db 9E
    .db 48
    .db D0
    .db 06
    .db 10
    .db 00
    .db 09
    .db 02
    .db 18
    .db A0
    .db 06
    .db 10
    .db 00
    .db 09
    .db 02
    .db 28
    .db D0
    .db 46
    .db 40
    .db 10
    .db 48
    .db 02
    .db 48
    .db D0
    .db 06
    .db FC
    .db 00
    .db 00
    .db 8D
    .db 20
    .db D0
    .db 06
    .db FC
    .db 04
    .db 00
    .db 24
    .db 48
    .db D0
    .db 47
    .db 20
    .db 10
    .db 30
    .db 04
    .db 48
    .db D0
    .db 07
    .db 50
    .db 00
    .db 00
    .db 9E
    .db 80
    .db 80
    .db 08
    .db 60
    .db 03
    .db 00
    .db BA
    .db 3C
    .db 80
    .db 08
    .db 60
    .db 03
    .db 00
    .db 25
    .db 3C
    .db 80
    .db 48
    .db 80
    .db 10
    .db 40
    .db 02
    .db 9C
    .db 80
    .db 09
    .db 40
    .db 03
    .db 00
    .db BA
    .db 9C
    .db 80
    .db 09
    .db 40
    .db 03
    .db 00
    .db 25
    .db 9C
    .db 80
    .db 49
    .db 80
    .db 10
    .db 50
    .db 05
    .db 9C
    .db 80
    .db 09
    .db C0
    .db 00
    .db 01
    .db 8B
    .db 20
    .db A0
    .db 0A
    .db 00
    .db 80
    .db 00
    .db 8F
    .db 80
    .db 70
    .db FF
    .db C3
    .db AE
    .db CA
    .db AE
    .db D1
    .db AE
    .db D8
    .db AE
    .db DF
    .db AE
    .db E6
    .db AE
    .db ED
    .db AE
    .db F4
    .db AE
    .db FB
    .db AE
    .db 02
    .db AF
    .db 09
    .db AF
    .db 10
    .db AF
    .db 17
    .db AF
    .db 1E
    .db AF
    .db 25
    .db AF
    .db 2C
    .db AF
    .db 33
    .db AF
    .db 3A
    .db AF
    .db 41
    .db AF
    .db 48
    .db AF
    .db 4F
    .db AF
    .db 56
    .db AF
    .db 5D
    .db AF
    .db 64
    .db AF
    .db 6B
    .db AF
    .db 72
    .db AF
    .db 79
    .db AF
    .db 80
    .db AF
    .db 87
    .db AF
    .db 8E
    .db AF
    .db 95
    .db AF
    .db 9C
    .db AF
    .db A3
    .db AF
    .db AA
    .db AF
    .db B1
    .db AF
    .db B8
    .db AF
    .db BF
    .db AF
    .db C6
    .db AF
    .db CD
    .db AF
    .db 00
    .db 00
    .db 00
    .db 0D
    .db 03
    .db 18
    .db A0
    .db 00
    .db 00
    .db 00
    .db 09
    .db 03
    .db 28
    .db C8
    .db 00
    .db 00
    .db 00
    .db 0A
    .db 03
    .db E0
    .db B0
    .db 40
    .db 40
    .db 10
    .db 48
    .db 03
    .db 80
    .db 70
    .db 00
    .db 80
    .db 00
    .db 01
    .db 09
    .db 18
    .db A0
    .db 00
    .db 80
    .db 00
    .db 01
    .db 09
    .db 18
    .db C0
    .db 40
    .db C0
    .db 10
    .db 60
    .db 09
    .db 28
    .db 70
    .db 01
    .db 00
    .db 00
    .db 00
    .db 90
    .db 80
    .db 70
    .db 01
    .db 20
    .db 00
    .db 01
    .db 08
    .db 18
    .db A0
    .db 01
    .db 20
    .db 00
    .db 01
    .db 08
    .db 28
    .db C8
    .db 41
    .db 40
    .db 10
    .db 60
    .db 08
    .db 80
    .db 70
    .db 01
    .db A0
    .db 00
    .db 00
    .db 32
    .db D0
    .db A0
    .db 01
    .db A0
    .db 00
    .db 00
    .db 32
    .db E0
    .db B8
    .db 01
    .db A0
    .db 00
    .db 00
    .db 32
    .db F0
    .db D0
    .db 41
    .db A0
    .db 10
    .db 60
    .db 0B
    .db C0
    .db A0
    .db 02
    .db 00
    .db 00
    .db 01
    .db 09
    .db 18
    .db A0
    .db 02
    .db 00
    .db 00
    .db 01
    .db 09
    .db 28
    .db C8
    .db 42
    .db 40
    .db 10
    .db 60
    .db 09
    .db C0
    .db A0
    .db 02
    .db E0
    .db 00
    .db 00
    .db 90
    .db 80
    .db 70
    .db 43
    .db 80
    .db 10
    .db 60
    .db 0D
    .db 30
    .db 98
    .db 03
    .db A0
    .db 00
    .db 00
    .db 26
    .db 80
    .db 40
    .db 05
    .db 00
    .db 00
    .db 01
    .db 10
    .db 30
    .db 88
    .db 05
    .db 00
    .db 00
    .db 01
    .db 10
    .db 30
    .db E0
    .db 05
    .db 00
    .db 00
    .db 00
    .db 90
    .db 80
    .db 70
    .db 05
    .db 40
    .db 00
    .db 01
    .db 02
    .db 18
    .db 88
    .db 05
    .db 40
    .db 00
    .db 01
    .db 02
    .db 28
    .db D0
    .db 05
    .db 40
    .db 00
    .db 02
    .db 02
    .db E0
    .db A0
    .db 45
    .db 48
    .db 10
    .db 50
    .db 02
    .db 80
    .db 70
    .db 05
    .db C0
    .db 00
    .db 01
    .db 08
    .db 18
    .db 88
    .db 45
    .db C8
    .db 10
    .db 60
    .db 08
    .db E8
    .db 88
    .db 06
    .db 50
    .db 00
    .db 0D
    .db 03
    .db 18
    .db 88
    .db 06
    .db 50
    .db 00
    .db 09
    .db 03
    .db 28
    .db C0
    .db 46
    .db 58
    .db 10
    .db 68
    .db 03
    .db 80
    .db 70
    .db 06
    .db 98
    .db 00
    .db 00
    .db C0
    .db E0
    .db 92
    .db 06
    .db 98
    .db 00
    .db 00
    .db C0
    .db F0
    .db B2
    .db 46
    .db 98
    .db 30
    .db 30
    .db 02
    .db D0
    .db C0
    .db 06
    .db A8
    .db 00
    .db 00
    .db C0
    .db F0
    .db D2
    .db 46
    .db A8
    .db B0
    .db 60
    .db 02
    .db D0
    .db C0
    .db FF
L_09:0FCE: ; 09:0FCE, 0x012FCE
    .db E4
L_09:0FCF: ; 09:0FCF, 0x012FCF
    .db AF
    .db E9
    .db AF
    .db EE
    .db AF
    .db F3
    .db AF
    .db F8
    .db AF
    .db FD
    .db AF
    .db 02
    .db B0
    .db 02
    .db B0
    .db 02
    .db B0
    .db 02
    .db B0
    .db 02
    .db B0
    .db 00
    .db 01
    .db 36
    .db 20
    .db 98
    .db 00
    .db 01
    .db 36
    .db 20
    .db 98
    .db 10
    .db 61
    .db 0E
    .db 20
    .db 98
    .db 10
    .db 61
    .db 0F
    .db 20
    .db 98
    .db 00
    .db 01
    .db 36
    .db 20
    .db 98
    .db 00
    .db 01
    .db 36
    .db 20
    .db 98
    .db FF
    .db 53
    .db B0
    .db 5A
    .db B0
    .db 61
    .db B0
    .db 68
    .db B0
    .db 6F
    .db B0
    .db 76
    .db B0
    .db 7D
    .db B0
    .db 84
    .db B0
    .db 8B
    .db B0
    .db 92
    .db B0
    .db 99
    .db B0
    .db A7
    .db B0
    .db AE
    .db B0
    .db B5
    .db B0
    .db BC
    .db B0
    .db C3
    .db B0
    .db CA
    .db B0
    .db D1
    .db B0
    .db D8
    .db B0
    .db DF
    .db B0
    .db E6
    .db B0
    .db ED
    .db B0
    .db F4
    .db B0
    .db FB
    .db B0
    .db 02
    .db B1
    .db 09
    .db B1
    .db 10
    .db B1
    .db 17
    .db B1
    .db 1E
    .db B1
    .db 25
    .db B1
    .db 2C
    .db B1
    .db 33
    .db B1
    .db 3A
    .db B1
    .db 41
    .db B1
    .db 48
    .db B1
    .db 4F
    .db B1
    .db 56
    .db B1
    .db 5D
    .db B1
    .db 64
    .db B1
    .db 6B
    .db B1
    .db 40
    .db 10
    .db 00
    .db 40
    .db 34
    .db 7C
    .db 80
    .db 40
    .db 40
    .db 10
    .db 70
    .db 02
    .db 80
    .db 80
    .db 40
    .db 80
    .db 00
    .db 40
    .db 34
    .db 8C
    .db 80
    .db 00
    .db C0
    .db 00
    .db 0D
    .db 03
    .db 20
    .db D0
    .db 00
    .db C0
    .db 00
    .db 0A
    .db 03
    .db E0
    .db C8
    .db 00
    .db C0
    .db 00
    .db 0A
    .db 03
    .db E0
    .db A4
    .db 40
    .db E0
    .db 30
    .db 68
    .db 03
    .db 20
    .db E0
    .db 20
    .db F0
    .db 00
    .db 00
    .db 93
    .db E0
    .db A0
    .db 21
    .db 40
    .db 00
    .db 00
    .db 93
    .db E0
    .db A0
    .db 21
    .db 9F
    .db 00
    .db 00
    .db 93
    .db E0
    .db A0
    .db 01
    .db A0
    .db 01
    .db 01
    .db 04
    .db 20
    .db A4
    .db 01
    .db A0
    .db 01
    .db 02
    .db 04
    .db E0
    .db A4
    .db 41
    .db D8
    .db 10
    .db 60
    .db 04
    .db 20
    .db E0
    .db 21
    .db FF
    .db 00
    .db 00
    .db 93
    .db E0
    .db A0
    .db 02
    .db 00
    .db 01
    .db 01
    .db 02
    .db 20
    .db A4
    .db 02
    .db 00
    .db 01
    .db 02
    .db 02
    .db E0
    .db A4
    .db 02
    .db 00
    .db 00
    .db 01
    .db 02
    .db 20
    .db D0
    .db 42
    .db 40
    .db 10
    .db 60
    .db 02
    .db 20
    .db E0
    .db 02
    .db A0
    .db 00
    .db 02
    .db 09
    .db E0
    .db D0
    .db 02
    .db A0
    .db 00
    .db 01
    .db 09
    .db 20
    .db D0
    .db 42
    .db C0
    .db 10
    .db 61
    .db 09
    .db 20
    .db E0
    .db 43
    .db 00
    .db 10
    .db 90
    .db 0A
    .db 20
    .db E0
    .db 03
    .db 60
    .db 00
    .db 00
    .db B5
    .db D0
    .db 90
    .db 43
    .db 60
    .db 10
    .db 50
    .db 02
    .db 20
    .db E0
    .db 04
    .db 40
    .db 00
    .db 00
    .db B5
    .db 70
    .db 90
    .db 44
    .db 40
    .db 10
    .db 60
    .db 02
    .db 40
    .db A0
    .db 45
    .db 09
    .db 00
    .db 00
    .db 5D
    .db 7F
    .db 8F
    .db 05
    .db 20
    .db 00
    .db 01
    .db 07
    .db 18
    .db 98
    .db 05
    .db 20
    .db 00
    .db 01
    .db 07
    .db 28
    .db C8
    .db 05
    .db 20
    .db 00
    .db 02
    .db 07
    .db E0
    .db 98
    .db 45
    .db 60
    .db 10
    .db 60
    .db 07
    .db 20
    .db E0
    .db 05
    .db C0
    .db 00
    .db 01
    .db 02
    .db 28
    .db A0
    .db 05
    .db C0
    .db 00
    .db 01
    .db 02
    .db 18
    .db D0
    .db 05
    .db C0
    .db 00
    .db 02
    .db 02
    .db E0
    .db C0
    .db 46
    .db 00
    .db 10
    .db 70
    .db 02
    .db 20
    .db E0
    .db 06
    .db 38
    .db 00
    .db 01
    .db 94
    .db 40
    .db E0
    .db 06
    .db 68
    .db 00
    .db 01
    .db 94
    .db 40
    .db E0
    .db 07
    .db 20
    .db 00
    .db 01
    .db 8B
    .db 10
    .db A0
    .db 07
    .db C0
    .db 00
    .db 00
    .db A6
    .db 10
    .db A0
    .db 08
    .db 00
    .db 80
    .db 00
    .db 5E
    .db 80
    .db 90
    .db FF
    .db E2
    .db B1
    .db E9
    .db B1
    .db F0
    .db B1
    .db F7
    .db B1
    .db FE
    .db B1
    .db 05
    .db B2
    .db 0C
    .db B2
    .db 13
    .db B2
    .db 1A
    .db B2
    .db 21
    .db B2
    .db 28
    .db B2
    .db 2F
    .db B2
    .db 36
    .db B2
    .db 3D
    .db B2
    .db 44
    .db B2
    .db 4B
    .db B2
    .db 52
    .db B2
    .db 59
    .db B2
    .db 60
    .db B2
    .db 67
    .db B2
    .db 6E
    .db B2
    .db 75
    .db B2
    .db 7C
    .db B2
    .db 83
    .db B2
    .db 8A
    .db B2
    .db 91
    .db B2
    .db 98
    .db B2
    .db 9F
    .db B2
    .db A6
    .db B2
    .db AD
    .db B2
    .db B4
    .db B2
    .db BB
    .db B2
    .db C2
    .db B2
    .db C9
    .db B2
    .db D0
    .db B2
    .db D7
    .db B2
    .db DE
    .db B2
    .db E5
    .db B2
    .db EC
    .db B2
    .db F3
    .db B2
    .db FA
    .db B2
    .db 01
    .db B3
    .db 08
    .db B3
    .db 0F
    .db B3
    .db 16
    .db B3
    .db 1D
    .db B3
    .db 24
    .db B3
    .db 2B
    .db B3
    .db 32
    .db B3
    .db 39
    .db B3
    .db 40
    .db B3
    .db 47
    .db B3
    .db 4E
    .db B3
    .db 55
    .db B3
    .db 5C
    .db B3
    .db 63
    .db B3
    .db 64
    .db B3
    .db 6B
    .db B3
    .db 72
    .db B3
    .db 00
    .db 10
    .db 00
    .db 01
    .db 07
    .db 28
    .db A8
    .db 00
    .db 10
    .db 00
    .db 01
    .db 07
    .db 18
    .db D0
    .db 40
    .db 38
    .db 10
    .db 60
    .db 07
    .db 28
    .db A8
    .db 00
    .db 40
    .db 00
    .db 01
    .db 92
    .db 40
    .db E0
    .db 00
    .db 60
    .db 00
    .db 01
    .db 92
    .db 40
    .db E0
    .db 01
    .db 20
    .db 00
    .db 00
    .db 99
    .db F0
    .db 80
    .db 01
    .db A0
    .db 00
    .db 01
    .db 06
    .db 28
    .db A8
    .db 01
    .db A0
    .db 00
    .db 01
    .db 06
    .db 18
    .db D0
    .db 01
    .db A0
    .db 00
    .db 02
    .db 06
    .db E0
    .db C0
    .db 41
    .db B0
    .db 10
    .db 60
    .db 06
    .db 28
    .db A8
    .db 01
    .db E0
    .db 05
    .db 01
    .db 9A
    .db 10
    .db 90
    .db 42
    .db 78
    .db 10
    .db 60
    .db 02
    .db 28
    .db A0
    .db 02
    .db 80
    .db 00
    .db 01
    .db 96
    .db 10
    .db D0
    .db 02
    .db B0
    .db 00
    .db 01
    .db 96
    .db 10
    .db B0
    .db 02
    .db E0
    .db 00
    .db 01
    .db 96
    .db 10
    .db 90
    .db 03
    .db 10
    .db 00
    .db 01
    .db 96
    .db 10
    .db C0
    .db 03
    .db 40
    .db 00
    .db 01
    .db 96
    .db 10
    .db 90
    .db 04
    .db 60
    .db 00
    .db 0D
    .db 03
    .db 18
    .db A0
    .db 04
    .db 60
    .db 00
    .db 09
    .db 03
    .db 28
    .db D0
    .db 04
    .db 60
    .db 00
    .db 0A
    .db 03
    .db E0
    .db C0
    .db 44
    .db 70
    .db 10
    .db 78
    .db 03
    .db 28
    .db A8
    .db 44
    .db A0
    .db 10
    .db E0
    .db 0A
    .db 28
    .db A8
    .db 04
    .db B0
    .db 00
    .db 01
    .db 92
    .db 40
    .db E0
    .db 04
    .db D0
    .db 00
    .db 01
    .db 92
    .db 40
    .db E0
    .db 06
    .db 10
    .db 00
    .db 01
    .db 04
    .db 18
    .db A0
    .db 06
    .db 10
    .db 00
    .db 01
    .db 04
    .db 28
    .db D0
    .db 46
    .db 20
    .db 10
    .db C0
    .db 04
    .db 28
    .db A8
    .db 06
    .db 60
    .db 00
    .db 01
    .db 06
    .db 28
    .db A0
    .db 06
    .db 60
    .db 00
    .db 01
    .db 06
    .db 18
    .db D0
    .db 06
    .db 60
    .db 00
    .db 02
    .db 06
    .db E0
    .db C0
    .db 06
    .db A0
    .db 00
    .db 00
    .db 99
    .db F0
    .db 80
    .db 46
    .db D0
    .db 10
    .db C0
    .db 06
    .db 28
    .db A8
    .db 06
    .db E0
    .db 00
    .db 01
    .db 96
    .db 10
    .db 90
    .db 07
    .db 10
    .db 00
    .db 01
    .db 96
    .db 10
    .db 90
    .db 07
    .db 40
    .db 00
    .db 01
    .db 96
    .db 10
    .db 90
    .db 07
    .db 70
    .db 00
    .db 01
    .db 96
    .db 10
    .db 90
    .db 09
    .db 00
    .db 00
    .db 00
    .db 9D
    .db 10
    .db 90
    .db 09
    .db 00
    .db 00
    .db 00
    .db 9B
    .db 10
    .db 90
    .db 09
    .db 10
    .db 00
    .db 01
    .db 8B
    .db 10
    .db A4
    .db 09
    .db 80
    .db 00
    .db 01
    .db 04
    .db 28
    .db A0
    .db 09
    .db 80
    .db 00
    .db 01
    .db 04
    .db 18
    .db D0
    .db 49
    .db C0
    .db 10
    .db C0
    .db 04
    .db 28
    .db A8
    .db 0A
    .db 30
    .db 00
    .db 00
    .db 32
    .db D0
    .db A0
    .db 0A
    .db 30
    .db 00
    .db 00
    .db 32
    .db E0
    .db B8
    .db 0A
    .db 30
    .db 00
    .db 00
    .db 32
    .db F0
    .db D0
    .db 4A
    .db 30
    .db 10
    .db 4C
    .db 03
    .db D0
    .db A0
    .db 0A
    .db 50
    .db 00
    .db 00
    .db 32
    .db D0
    .db A0
    .db 0A
    .db 50
    .db 00
    .db 00
    .db 32
    .db E0
    .db B8
    .db 0A
    .db 50
    .db 00
    .db 00
    .db 32
    .db F0
    .db D0
    .db 4A
    .db 50
    .db 10
    .db 40
    .db 08
    .db D0
    .db A0
    .db 0A
    .db 80
    .db 00
    .db 01
    .db 02
    .db 18
    .db A0
    .db 0A
    .db 80
    .db 00
    .db 01
    .db 02
    .db 28
    .db D0
    .db 0A
    .db 80
    .db 00
    .db 02
    .db 02
    .db E0
    .db A8
    .db 4A
    .db A0
    .db 10
    .db C0
    .db 02
    .db 28
    .db A8
    .db 4A
    .db FE
    .db 80
    .db 01
    .db 62
    .db 20
    .db C0
    .db FF
    .db 00
    .db 00
    .db 40
    .db 00
    .db 67
    .db 80
    .db A0
    .db 00
    .db 00
    .db C0
    .db 00
    .db 6D
    .db 80
    .db A0
    .db FF
    .db AD
    .db B3
    .db B4
    .db B3
    .db BB
    .db B3
    .db C2
    .db B3
    .db C9
    .db B3
    .db D0
    .db B3
    .db D7
    .db B3
    .db DE
    .db B3
    .db E5
    .db B3
    .db EC
    .db B3
    .db F3
    .db B3
    .db FA
    .db B3
    .db 01
    .db B4
    .db 08
    .db B4
    .db 0F
    .db B4
    .db 16
    .db B4
    .db 1D
    .db B4
    .db 24
    .db B4
    .db 2B
    .db B4
    .db 32
    .db B4
    .db 39
    .db B4
    .db 40
    .db B4
    .db 47
    .db B4
    .db 4E
    .db B4
    .db 55
    .db B4
    .db 5C
    .db B4
    .db 63
    .db B4
    .db 6A
    .db B4
    .db 71
    .db B4
    .db 00
    .db 20
    .db 00
    .db 00
    .db A2
    .db F0
    .db 40
    .db 01
    .db 70
    .db 00
    .db 01
    .db 02
    .db 18
    .db A8
    .db 01
    .db 74
    .db 00
    .db 02
    .db 02
    .db E8
    .db C0
    .db 01
    .db 78
    .db 00
    .db 01
    .db 02
    .db 20
    .db D0
    .db 41
    .db 7C
    .db 10
    .db 60
    .db 02
    .db 18
    .db A8
    .db 41
    .db C0
    .db 00
    .db 00
    .db A4
    .db 80
    .db 40
    .db 43
    .db 00
    .db 10
    .db 30
    .db 49
    .db 08
    .db D0
    .db 03
    .db 80
    .db 00
    .db 01
    .db 12
    .db 18
    .db A8
    .db 03
    .db 84
    .db 00
    .db 02
    .db 12
    .db E8
    .db C0
    .db 03
    .db 88
    .db 00
    .db 01
    .db 12
    .db 28
    .db D0
    .db 43
    .db 8C
    .db 10
    .db 40
    .db 12
    .db 08
    .db D0
    .db 04
    .db 94
    .db 00
    .db 01
    .db 02
    .db 18
    .db A8
    .db 04
    .db 98
    .db 00
    .db 02
    .db 02
    .db E8
    .db C8
    .db 04
    .db 9C
    .db 00
    .db 01
    .db 02
    .db 28
    .db D0
    .db 44
    .db A0
    .db 10
    .db 60
    .db 02
    .db E0
    .db 40
    .db 45
    .db C0
    .db 00
    .db 00
    .db A4
    .db 80
    .db 40
    .db 06
    .db 00
    .db 00
    .db 01
    .db 12
    .db 18
    .db A8
    .db 06
    .db 04
    .db 00
    .db 02
    .db 12
    .db E8
    .db C8
    .db 06
    .db 08
    .db 00
    .db 01
    .db 12
    .db 28
    .db D8
    .db 46
    .db 0C
    .db 10
    .db 60
    .db 12
    .db 18
    .db A8
    .db 46
    .db 80
    .db 10
    .db 40
    .db 49
    .db 80
    .db 40
    .db 07
    .db 20
    .db 00
    .db 00
    .db A2
    .db F0
    .db 40
    .db 08
    .db 74
    .db 00
    .db 01
    .db 02
    .db 18
    .db B0
    .db 08
    .db 78
    .db 00
    .db 02
    .db 02
    .db E8
    .db A0
    .db 08
    .db 7C
    .db 00
    .db 01
    .db 02
    .db 18
    .db D0
    .db 48
    .db 80
    .db 10
    .db 70
    .db 02
    .db 80
    .db 40
    .db 08
    .db 84
    .db 00
    .db 00
    .db A5
    .db 80
    .db 40
    .db 09
    .db 00
    .db C0
    .db 00
    .db 75
    .db D0
    .db C0
    .db FF
    .db BA
    .db B4
    .db C1
    .db B4
    .db C8
    .db B4
    .db CF
    .db B4
    .db D6
    .db B4
    .db DD
    .db B4
    .db E4
    .db B4
    .db EB
    .db B4
    .db F2
    .db B4
    .db F9
    .db B4
    .db 00
    .db B5
    .db 07
    .db B5
    .db 0E
    .db B5
    .db 15
    .db B5
    .db 1C
    .db B5
    .db 23
    .db B5
    .db 2A
    .db B5
    .db 31
    .db B5
    .db 38
    .db B5
    .db 3F
    .db B5
    .db 46
    .db B5
    .db 4D
    .db B5
    .db 54
    .db B5
    .db 5B
    .db B5
    .db 62
    .db B5
    .db 69
    .db B5
    .db 70
    .db B5
    .db 77
    .db B5
    .db 7E
    .db B5
    .db 85
    .db B5
    .db 8C
    .db B5
    .db 93
    .db B5
    .db 9A
    .db B5
    .db A1
    .db B5
    .db A8
    .db B5
    .db AF
    .db B5
    .db 00
    .db C0
    .db 00
    .db 00
    .db BD
    .db 70
    .db 8C
    .db 40
    .db C8
    .db 10
    .db 70
    .db 02
    .db 10
    .db D0
    .db 41
    .db 40
    .db 10
    .db 70
    .db 4D
    .db 10
    .db D0
    .db 01
    .db C0
    .db 00
    .db 00
    .db BD
    .db 70
    .db 8C
    .db 41
    .db C8
    .db 10
    .db 70
    .db 02
    .db 10
    .db D0
    .db 03
    .db 00
    .db 0A
    .db 0C
    .db BE
    .db 78
    .db C8
    .db 43
    .db 08
    .db 10
    .db 68
    .db 02
    .db E0
    .db D0
    .db 03
    .db E0
    .db 0A
    .db 0C
    .db BE
    .db 98
    .db A8
    .db 03
    .db E8
    .db 0A
    .db 08
    .db BE
    .db A0
    .db B8
    .db 03
    .db F0
    .db 0A
    .db 08
    .db BE
    .db 88
    .db C8
    .db 44
    .db 00
    .db 10
    .db 68
    .db 02
    .db E0
    .db D0
    .db 04
    .db 66
    .db 00
    .db 09
    .db 9F
    .db 10
    .db A0
    .db 04
    .db C8
    .db 00
    .db 0A
    .db 02
    .db E8
    .db C0
    .db 04
    .db D0
    .db 00
    .db 09
    .db 02
    .db 18
    .db D0
    .db 05
    .db 0A
    .db 00
    .db 09
    .db 9F
    .db 10
    .db A0
    .db 45
    .db 10
    .db 10
    .db 69
    .db 02
    .db 10
    .db D0
    .db 05
    .db 60
    .db 00
    .db 09
    .db 02
    .db 10
    .db D0
    .db 05
    .db 64
    .db 00
    .db 0A
    .db 02
    .db E8
    .db C0
    .db 05
    .db 68
    .db 00
    .db 09
    .db 02
    .db 10
    .db B0
    .db 45
    .db 70
    .db 10
    .db 68
    .db 02
    .db 10
    .db D0
    .db 06
    .db 20
    .db 00
    .db 00
    .db A0
    .db E8
    .db D0
    .db 06
    .db 70
    .db 00
    .db 00
    .db A0
    .db E8
    .db D0
    .db 06
    .db C0
    .db 00
    .db 00
    .db A0
    .db E8
    .db D0
    .db 47
    .db E0
    .db 00
    .db 00
    .db 3B
    .db 40
    .db 8C
    .db 48
    .db 20
    .db 00
    .db 00
    .db 3B
    .db C0
    .db 8C
    .db 49
    .db 00
    .db 10
    .db 40
    .db 3E
    .db 10
    .db D0
    .db 49
    .db 80
    .db 10
    .db 60
    .db 4D
    .db 10
    .db D0
    .db 0A
    .db 00
    .db 0B
    .db 00
    .db BF
    .db 30
    .db 8D
    .db 4A
    .db 08
    .db 10
    .db 60
    .db 02
    .db E0
    .db D0
    .db 0A
    .db 20
    .db 0B
    .db 00
    .db BF
    .db 50
    .db 8D
    .db 4A
    .db 28
    .db 10
    .db 60
    .db 02
    .db E0
    .db D0
    .db 0A
    .db 50
    .db 00
    .db 01
    .db 8B
    .db 20
    .db A8
    .db 0A
    .db 80
    .db 00
    .db 00
    .db BD
    .db 70
    .db 8C
    .db 4A
    .db 88
    .db 10
    .db 70
    .db 02
    .db 10
    .db D0
    .db 0B
    .db 00
    .db C0
    .db 00
    .db 70
    .db 10
    .db A0
    .db FF
OBJ_STATE_0x36_HANDLER: ; 09:15B0, 0x0135B0
    LDA 5FD_UNK ; Load
    BNE 5FD_NONZERO ; != 0, goto.
    LDA #$05
    STA 5FD_UNK ; Set to val.
    LDA #$07
    JSR SND_BANKED_DISPATCH ; Play sound.
5FD_NONZERO: ; 09:15BF, 0x0135BF
    DEC 5FD_UNK ; --
    JSR ADD_TO_VAL_5EE/5EF ; Do.
    LDA IRQ/SCRIPT_RUN_COUNT? ; Load
    LSR A ; >> 1, /2.
    BCC DO_SECONDARY_SWITCH ; CC.c == 0, goto.
    LDA OBJ_SECONDARY_SWITCH?[18],X ; Load
    CMP #$04 ; If _ #$04
    BCS DO_SECONDARY_SWITCH ; >=, goto.
    JSR OBJ_RTN_UNK_RET_VAL_UNK ; Do.
    CMP #$00 ; If A _ #$00
    BEQ DO_SECONDARY_SWITCH ; ==, goto.
    LDA #$04
    STA OBJ_SECONDARY_SWITCH?[18],X ; Set secondary.
    LDA #$00
    STA OBJ_TERTIARY_SWITCH?[18],X ; Clear tert.
    JMP OBJECT_X_MOVE? ; Exit move.
DO_SECONDARY_SWITCH: ; 09:15E5, 0x0135E5
    LDA OBJ_SECONDARY_SWITCH?[18],X ; Do switch.
    JSR SWITCH_CODE_PTRS_PAST_JSR
    LOW(UNK_SWITCH3_A)
    HIGH(UNK_SWITCH3_A)
    LOW(UNK_SWITCH3_B)
    HIGH(UNK_SWITCH3_B)
    LOW(UNK_SWITCH3_C)
    HIGH(UNK_SWITCH3_C)
    LOW(UNK_SWITCH3_D) ; Spawns stuff, 5F0 things.
    HIGH(UNK_SWITCH3_D)
    LOW(UNK_SWITCH3_E)
    HIGH(UNK_SWITCH3_E)
    LOW(UNK_SWITCH3_F)
    HIGH(UNK_SWITCH3_F)
UNK_SWITCH3_B: ; 09:15F7, 0x0135F7
    JSR FIND_PLAYER_FOCUS_HELPER ; Do sub.
    INC OBJ_SECONDARY_SWITCH?[18],X ; Move secondary.
    JMP OBJECT_X_MOVE? ; Exit, abuse RTS.
UNK_SWITCH3_A: ; 09:1600, 0x013600
    LDA OBJ_TERTIARY_SWITCH?[18],X ; Load tert.
    BNE TERT_NONZERO ; != 0, goto.
    LDA #$40
    STA 59E_OBJ_UNK/EXTRA_TIMER[18],X ; Set attr.
    LDA #$59
    STA SCRIPT_R0-R5_GFX_BANK_VALS+4 ; Set GFX.
    LDA #$58
    STA SCRIPT_R0-R5_GFX_BANK_VALS+5
    LDY #$11
    JSR UPDATE_PALETTE[0x1C]_WITH_Y_SAVING_XOBJ ; Set color.
    INC OBJ_TERTIARY_SWITCH?[18],X ; Tert++
    LDY #$00 ; File to use.
    JSR FILL_ATTRS_FROM_FILE ; Set attrs.
    JMP OBJECT_X_MOVE? ; Exit, abuse RTS.
TERT_NONZERO: ; 09:1622, 0x013622
    DEC 59E_OBJ_UNK/EXTRA_TIMER[18],X
    BNE 09:165D
    LDA #$10
    STA OBJECT_DATA_HEALTH?[18],X
    STA OBJECT_DATA_EXTRA_B?[18],X
    LDA #$4E
    STA OBJ_ANIMATION_DISPLAY[18],X
    LDA #$B0
    STA 4C6_OBJ_UNK[18],X
    LDA #$37
    STA OBJ_ENABLED_STATE+MORE?+1,X
    LDA #$00
    STA 5EE_UNK
    STA 5EF_UNK
    LDA 556_OBJ_STATUS_FLAGS_A[18],X
    AND #$04
    LSR A
    LSR A
    STA 5F6_UNK
    LDY #$02
    JSR FILL_ATTRS_FROM_FILE
    LDA #$00
    STA OBJ_TERTIARY_SWITCH?[18],X
    INC OBJ_SECONDARY_SWITCH?[18],X
    JMP OBJECT_X_MOVE?
UNK_SWITCH3_C: ; 09:1660, 0x013660
    LDA OBJ_TERTIARY_SWITCH?[18],X
    JSR SWITCH_CODE_PTRS_PAST_JSR
    LOW(SWITCH_RTN_A)
    HIGH(SWITCH_RTN_A)
    LOW(SWITCH_RTN_B)
    HIGH(SWITCH_RTN_B)
    LOW(SWITCH_RTN_C)
    HIGH(SWITCH_RTN_C)
SWITCH_RTN_A: ; 09:166C, 0x01366C
    JSR SET_ANIM_W/_HOLD/ENTRY_MOD ; dO.
    LDA 5F1_UNK ; Load
    AND #$7F ; Keep 0111.1111
    STA 5F1_UNK ; Store back.
    LDY #$00 ; File.
    JSR FILL_ATTRS_FROM_FILE ; Do.
    INC OBJ_TERTIARY_SWITCH?[18],X ; Tert++
    LDA IRQ/SCRIPT_RUN_COUNT? ; Load
    BPL EXIT_MOVE ; Positive, skip to.
    INC OBJ_TERTIARY_SWITCH?[18],X ; Tert inc.
EXIT_MOVE: ; 09:1686, 0x013686
    JMP OBJECT_X_MOVE? ; Abuse RTS.
SWITCH_RTN_B: ; 09:1689, 0x013689
    JSR CLEAR_OBJ_ATTRS_UNK_1546 ; Clear attrs.
    JSR CLEAR_OBJ_ATTRS_UNK_154F
    JSR SET_ANIM_W/_HOLD/ENTRY_MOD ; Set anim.
    JSR SUB_ATTR_CLEAR_DELTA_MORE ; Do sub.
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load
    AND #$03 ; Keep 0000.0011
    BEQ NO_BITS_SET ; == 0, goto.
    AND #$01 ; Test 0x01
    BEQ A_SEED_0x01 ; == 0, goto.
    LDA #$00 ; Seed.
    BEQ A_SEEDED ; Always taken, goto.
A_SEED_0x01: ; 09:16A4, 0x0136A4
    LDA #$01 ; Seed
A_SEEDED: ; 09:16A6, 0x0136A6
    STA 5F6_UNK ; Store to.
    LDY #$02 ; File.
    JSR FILL_ATTRS_FROM_FILE ; Fill in.
    JMP RTN_POS_DIFF_EXIT ; Exit rtn.
NO_BITS_SET: ; 09:16B1, 0x0136B1
    LDY STATUS_FLAGS_B/OBJ_FOCUS[18],X ; Load Yobj from Xobj.
    JSR FIND_XPOS_DIFF_OBJY/OBJX_TMP_12_FLAG_XVAL_GT_YVAL ; Find.
    STA TMP_16 ; Store result.
    LDA TMP_12 ; Load
    BNE A_SEED_0x00 ; != 0, goto.
    LDA #$01 ; Seed.
    BNE A_SEEDED ; Always taken.
A_SEED_0x00: ; 09:16C1, 0x0136C1
    LDA #$00 ; Seed.
A_SEEDED: ; 09:16C3, 0x0136C3
    STA 5F6_UNK ; Store to.
    LDY #$02 ; File.
    JSR FILL_ATTRS_FROM_FILE ; Put.
    JSR RTN_POS_DIFF_EXIT ; Exit as sub? TODO: Okay to do?
    STA TMP_17 ; Store return...
    CMP #$08 ; If _ #$08
    BCC LT_VAL ; <, goto.
    JMP EXIT_SEC_++_TERT_0x00 ; Exit moving secondary/clearing tert.
LT_VAL: ; 09:16D7, 0x0136D7
    RTS ; Leave.
SWITCH_RTN_C: ; 09:16D8, 0x0136D8
    JSR CLEAR_OBJ_ATTRS_UNK_1546 ; Clear attrs.
    JSR CLEAR_OBJ_ATTRS_UNK_154F
    JSR SET_ANIM_W/_HOLD/ENTRY_MOD ; Animation set.
    JSR SUB_ATTR_CLEAR_DELTA_MORE ; Do.
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load
    AND #$03 ; Keep 0000.0011
    BEQ NO_BITS_SET
    AND #$01 ; Test 0x01
    BEQ SEED_0x01
    LDA #$00 ; Seed.
    BEQ A_SEEDED
SEED_0x01: ; 09:16F3, 0x0136F3
    LDA #$01
A_SEEDED: ; 09:16F5, 0x0136F5
    STA 5F6_UNK ; Set.
    LDY #$04 ; File.
    JSR FILL_ATTRS_FROM_FILE ; Do.
    JMP RTN_POS_DIFF_EXIT ; Do, exit.
NO_BITS_SET: ; 09:1700, 0x013700
    LDY STATUS_FLAGS_B/OBJ_FOCUS[18],X
    JSR FIND_XPOS_DIFF_OBJY/OBJX_TMP_12_FLAG_XVAL_GT_YVAL
    STA TMP_16
    LDA TMP_12
    BNE 09:1710
    LDA #$01
    BNE 09:1712
    LDA #$00
    STA 5F6_UNK
    LDY #$04
    JSR FILL_ATTRS_FROM_FILE
    JSR RTN_POS_DIFF_EXIT
    LDA TMP_17
    CMP #$08
    BCS 09:172B
EXIT_SEC_++_TERT_0x00: ; 09:1723, 0x013723
    INC OBJ_SECONDARY_SWITCH?[18],X
    LDA #$00
    STA OBJ_TERTIARY_SWITCH?[18],X
    RTS
RTN_POS_DIFF_EXIT: ; 09:172C, 0x01372C
    LDY STATUS_FLAGS_B/OBJ_FOCUS[18],X ; Y from Xobj.
    JSR FIND_YPOS_DIFF_OBJY/OBJS_TMP_10_FLAG_XVAL_GT_YVAL ; Get diff.
    CMP #$08 ; If _ #$08
    BCC EXIT_MOVE ; <, goto.
    JSR RTN_ATTRS_DEPENDENT_ON_TMP_10 ; Do rtn.
EXIT_MOVE: ; 09:1739, 0x013739
    JMP OBJECT_X_MOVE? ; Abuse RTS.
SUB_ATTR_CLEAR_DELTA_MORE: ; 09:173C, 0x01373C
    JSR CLEAR_OBJ_ATTRS_UNK_154F ; Clear attrs.
    INC 59E_OBJ_UNK/EXTRA_TIMER[18],X ; ++
    LDA #$00
    STA 532_OBJ_UNK_POS_DELTA?[18],X ; Clear.
    LDA #$40
    STA 544_OBJ_UNK_POS_DELTA?[18],X ; Set attr.
    LDA 59E_OBJ_UNK/EXTRA_TIMER[18],X ; Load
    AND #$40 ; Keep 0100.0000
    BEQ EXIT_MOVE ; Bit not set, goto.
    LDA #$FF
    STA 532_OBJ_UNK_POS_DELTA?[18],X ; Set.
    LDA #$C0
    STA 544_OBJ_UNK_POS_DELTA?[18],X ; Set.
EXIT_MOVE: ; 09:175D, 0x01375D
    JMP MOVE_OBJ_ATTR_BY_DELTA_UNK ; Abuse RTS.
    JSR CLEAR_OBJ_ATTRS_DELTAS?
    INC 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X
    BNE 09:176B
    INC 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X
    LDA #$00
    STA 503_OBJ_POS_X_LARGEST?[18],X
    LDA #$40
    STA 520_OBJ_POS_X_LARGE?[18],X
    LDA 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X
    LSR A
    BCC 09:1785
    LDA #$FF
    STA 503_OBJ_POS_X_LARGEST?[18],X
    LDA #$C0
    STA 520_OBJ_POS_X_LARGE?[18],X
    RTS
UNK_SWITCH3_D: ; 09:1786, 0x013786
    DEC 5F7_UNK ; --
    BNE VAL_NE_ZERO ; != 0, goto.
    JSR SET_ANIM_W/_HOLD/ENTRY_MOD ; Anim.
    JSR CLEAR_OBJ_ATTRS_DELTAS? ; Clear attrs.
    LDA #$01
    STA OBJ_SECONDARY_SWITCH?[18],X ; Set secondary.
    JMP OBJECT_X_MOVE? ; Exit, abuse RTS.
VAL_NE_ZERO: ; 09:1799, 0x013799
    LDA 5F1_UNK ; Load
    ORA #$80 ; Set bit.
    STA 5F1_UNK ; Store back.
    LDA 5EF_UNK ; Load
    AND #$01 ; Test 0x01
    BNE BIT_SET ; != 0, goto. Set.
    JSR 09:1760
    JMP 09:17B1
BIT_SET: ; 09:17AE, 0x0137AE
    JSR SUB_ATTR_CLEAR_DELTA_MORE ; Clear sub.
    JSR SET_ANIM_W/_HOLD/ENTRY_MOD ; Anim.
    LDA OBJ_POS_X_SUBPIXEL_DELTA? ; Load from B7 val.
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; Store to Xobj.
    LDA OBJ_POS_X_DELTA? ; Load from BA val.
    STA OBJ_POS_X_DELTA?[18],X ; Store to Xobj.
    LDY STATUS_FLAGS_B/OBJ_FOCUS[18],X ; Y from Xobj.
    JSR FIND_XPOS_DIFF_OBJY/OBJX_TMP_12_FLAG_XVAL_GT_YVAL ; Get diff.
    CMP #$20 ; If _ #$20
    BCC 5F1_SET_0x01 ; <, goto.
    LDA TMP_12 ; Load
    BNE TMP_12_VAL ; != 0, goto.
ENTER_5F1_CODE: ; 09:17CC, 0x0137CC
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load
    AND #$04 ; Keep 0000.0100
    BNE BIT_0x04_SET ; != 0, set, goto.
    LDA #$01
    STA OBJ_SECONDARY_SWITCH?[18],X ; Set secondary.
BIT_0x04_SET: ; 09:17D8, 0x0137D8
    CLC ; Prep add.
    LDA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; Load
    ADC #$40 ; Add with.
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; Store back.
    LDA OBJ_POS_X_DELTA?[18],X ; Load
    ADC #$00 ; Carry add.
    STA OBJ_POS_X_DELTA?[18],X ; Store back.
    JMP REENTER ; Goto.
5F1_SET_0x01: ; 09:17EC, 0x0137EC
    LDA 5F1_UNK ; Load
    ORA #$01 ; Set
    STA 5F1_UNK ; Store back.
    LDA TMP_12 ; Load
    BNE REENTER ; != 0, goto. 
    BEQ ENTER_5F1_CODE ; Always taken.
TMP_12_VAL: ; 09:17FA, 0x0137FA
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load flags.
    AND #$04 ; Keep 0x04
    BEQ 5F1_CLEAR_0x01 ; == 0, not set, goto.
    LDA #$01
    STA OBJ_SECONDARY_SWITCH?[18],X ; Set to secondary.
5F1_CLEAR_0x01: ; 09:1806, 0x013806
    LDA 5F1_UNK ; Load
    AND #$FE ; Keep 1111.1110
    STA 5F1_UNK ; Store back.
    CLC ; Prep add.
    LDA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; Load.
    ADC #$C0 ; Add with.
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; Store back.
    LDA OBJ_POS_X_DELTA?[18],X ; Load.
    ADC #$FF ; Carry add, sub 1/2.
    STA OBJ_POS_X_DELTA?[18],X ; Store back.
REENTER: ; 09:181F, 0x01381F
    LDA 5F0_UNK ; Load
    CMP #$08 ; If _ #$08
    BCC EXIT_5F0_INC ; <, goto.
    LDA #$00
    STA 5F0_UNK ; Clear.
    JSR RTN_GET_OBJ_PAIR_IN_Y_RET_CC_SUCCESS ; Get.
    BCS RTN_FAIL ; Ret CS, fail, goto.
    STY TMP_00 ; Store obj ret here.
    LDA 5F1_UNK ; Load
    ASL A ; << 1, *2.
    TAY ; To Y index.
    LDA FILE_PTRS_L,Y ; Move File PTR into place.
    STA TMP_08
    LDA FILE_PTRS_H,Y
    STA TMP_09
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load
    LSR A ; >> 1, /2
    TAY ; To Y index.
    LDA [TMP_08],Y ; Load
    STA TMP_0A ; Seed TMP with stream data.
    INY
    LDA [TMP_08],Y
    STA TMP_0B
    LDY #$00 ; Stream ptr index reset.
    LDA [TMP_0A],Y
    STA TMP_10
    INY
    LDA [TMP_0A],Y
    STA TMP_11
    INY
    LDA [TMP_0A],Y
    STA TMP_12
    LDY TMP_00 ; Load YObj got earlier.
    LDA OBJ_POS_X??[18],X ; Load from Xobj.
    STA OBJ_POS_X??[18],Y ; Store to Yobj.
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Do again.
    STA 556_OBJ_STATUS_FLAGS_A[18],Y
    LDA OBJ_POS_X_CONFIRMED[18],X ; Load from Xobj.
    CLC ; Prep add.
    ADC TMP_11 ; Add with.
    STA OBJ_POS_X_CONFIRMED[18],Y ; Store to Yobj.
    LDA 4C6_OBJ_UNK[18],X ; Load from Xobj.
    CLC ; Prep add.
    ADC TMP_12 ; Add with.
    STA 4C6_OBJ_UNK[18],Y ; Store to Yobj.
    LDA TMP_10
    STA OBJ_ENABLED_STATE+MORE?[18],Y ; Set Yobj.
    LDA OBJ_POS_X_CONFIRMED[18],X ; Load
    BMI VAL_NEGATIVE ; Negative, goto.
    LDA OBJ_POS_X_CONFIRMED[18],Y ; Load from Yobj.
    CMP #$E0 ; If _ 0xE0
    BCC EXIT_5F0_INC ; <, goto.
    LDA #$02 ; Seed A.
    BNE A_SEEDED ; Always taken.
VAL_NEGATIVE: ; 09:1894, 0x013894
    LDA OBJ_POS_X_CONFIRMED[18],Y ; Load Xobj.
    CMP #$20 ; If A _ 0x20
    BCS EXIT_5F0_INC ; >=, goto.
    LDA #$01 ; Seed A.
A_SEEDED: ; 09:189D, 0x01389D
    CLC ; Prep add.
    ADC 556_OBJ_STATUS_FLAGS_A[18],Y ; Add with.
    STA 556_OBJ_STATUS_FLAGS_A[18],Y ; Store to.
EXIT_5F0_INC: ; 09:18A4, 0x0138A4
    INC 5F0_UNK ; Inc
RTN_FAIL: ; 09:18A7, 0x0138A7
    JSR RTN_POS_DIFF_EXIT ; Do sub.
    JMP OBJECT_X_MOVE? ; Goto, abuse RTS.
FILE_PTRS_L: ; 09:18AD, 0x0138AD
    .db B1
FILE_PTRS_H: ; 09:18AE, 0x0138AE
    .db B8
    .db B5
    .db B8
    .db B9
    .db B8
    .db BC
    .db B8
    .db BF
    .db B8
    .db BF
    .db B8
    .db 38
    .db E0
    .db 10
    .db 38
    .db 20
    .db 10
    .db 39
    .db 00
    .db 00
UNK_SWITCH3_E: ; 09:18C2, 0x0138C2
    LDA OBJ_TERTIARY_SWITCH?[18],X ; Load tert.
    BNE TERT_NONZERO ; != 0, goto.
    JSR CLEAR_OBJ_ATTRS_UNK_154F ; Clear attrs.
    LDA #$00
    STA 5F2_UNK ; Clear.
    LDA 5F1_UNK ; Load
    AND #$7F ; Keep 0111.1111
    STA 5F1_UNK ; Store back.
    JSR SET_ANIM_W/_HOLD/ENTRY_MOD ; Set anim.
    LDA 4C6_OBJ_UNK[18],X ; Load from Xobj.
    STA 5F5_UNK ; Store to.
    INC OBJ_TERTIARY_SWITCH?[18],X ; Tert++
    JSR OBJECT_X_MOVE? ; Move.
    LDA #$2C
    JMP SND_BANKED_DISPATCH ; Play sound, abuse RTS.
TERT_NONZERO: ; 09:18EB, 0x0138EB
    JSR SET_ANIM_W/_HOLD/ENTRY_MOD ; Set anim.
    LDA 5F2_UNK ; Load
    AND #$03 ; Keep 0000.0011
    BNE BITS_0x03_SET ; Any set, goto.
    INC OBJ_STATE_DIR_RELATED_C_SPR_DATA?[18],X ; ++
    LDA OBJ_STATE_DIR_RELATED_C_SPR_DATA?[18],X ; Load
    AND #$03 ; Keep 0000.0011
    STA OBJ_STATE_DIR_RELATED_C_SPR_DATA?[18],X ; Store back.
BITS_0x03_SET: ; 09:1900, 0x013900
    LDA 5F2_UNK ; Load
    AND #$01 ; Keep 0000.0001
    BNE 5F2_BIT_SET ; Set, goto.
    LDA 5F5_UNK ; Load
    SEC ; Prep sub.
    SBC #$02 ; Subtract val.
    JMP SUB_REENTER
5F2_BIT_SET: ; 09:1910, 0x013910
    LDA 5F5_UNK ; Load
    CLC ; Prep add.
    ADC #$02 ; Add val.
SUB_REENTER: ; 09:1916, 0x013916
    STA 4C6_OBJ_UNK[18],X ; Store to obj.
    INC 5F2_UNK ; ++
    LDA 5F2_UNK ; Load
    CMP #$40 ; If _ #$40
    BCC EXIT_MOVE ; <, goto.
    LDA #$00
    STA OBJ_STATE_DIR_RELATED_C_SPR_DATA?[18],X ; Clear.
    STA OBJ_TERTIARY_SWITCH?[18],X
    STA 5F3_UNK
    LDA OBJECT_DATA_HEALTH?[18],X ; Load
    BEQ HEALTH_ZERO ; == 0, goto.
    LDA #$02 ; Seed secondary switch.
    BNE SET_SWITCH_A_SEED
HEALTH_ZERO: ; 09:1937, 0x013937
    LDA #$05 ; Secondary switch seed.
SET_SWITCH_A_SEED: ; 09:1939, 0x013939
    STA OBJ_SECONDARY_SWITCH?[18],X ; Move secondary.
EXIT_MOVE: ; 09:193C, 0x01393C
    JMP OBJECT_X_MOVE? ; Goto, abuse RTS.
UNK_SWITCH3_F: ; 09:193F, 0x01393F
    LDA OBJ_TERTIARY_SWITCH?[18],X ; Load tert.
    BNE TERT_NONZERO ; != 0, goto.
    JSR CLEAR_OBJ_ATTRS_DELTAS? ; Clear attrs.
    JSR CLEAR_OBJ_ATTRS_UNK_154F
    LDA OBJ_POS_X_DELTA? ; Load 
    STA OBJ_POS_X_DELTA?[18],X ; Store to Xobj.
    LDA OBJ_POS_X_SUBPIXEL_DELTA? ; Load
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; Store to Xobj.
    LDA #$00
    STA 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Clear attrs.
    STA 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X
    INC OBJ_TERTIARY_SWITCH?[18],X ; Tert++
    JMP OBJECT_X_MOVE? ; Goto, abuse RTS.
TERT_NONZERO: ; 09:1962, 0x013962
    JSR CLEAR_OBJ_ATTRS_DELTAS? ; Clear attrs.
    LDA OBJ_POS_X_DELTA? ; Load
    STA OBJ_POS_X_DELTA?[18],X ; To Xobj.
    LDA OBJ_POS_X_SUBPIXEL_DELTA? ; Load
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; To Xobj.
    LDA 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X ; Load
    BEQ VAL_NONZERO
    DEC 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X ; --
EXIT_MOVE: ; 09:1977, 0x013977
    JMP OBJECT_X_MOVE? ; Goto, abuse RTS.
VAL_NONZERO: ; 09:197A, 0x01397A
    JSR RTN_GET_OBJ_PAIR_IN_Y_RET_CC_SUCCESS ; Do sub.
    BCS EXIT_MOVE ; CS, goto.
    STY TMP_10 ; Yobj to here.
    LDA 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Load
    ASL A ; << 1, *2.
    TAY ; To Y index.
    LDA TMP_DATA_A,Y ; Y index to 
    STA TMP_00
    LDA TMP_DATA_B,Y
    STA TMP_01
    LDY TMP_10 ; Get Yobj given again.
    LDA OBJ_POS_X_CONFIRMED[18],X ; Load from Xobj.
    CLC ; Prep add.
    ADC TMP_00 ; Add with.
    STA OBJ_POS_X_CONFIRMED[18],Y ; Store to Yobj.
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load
    AND #$03 ; Keep 0000.0011
    BEQ FLAGS_EQ_0x00 ; None set, goto.
    AND #$01 ; Test 0x01
    BEQ BIT_0x01_UNSET ; == 0, not set, goto.
    LDA TMP_00 ; Load val.
    BPL TMP_00_POSITIVE ; Positive, goto.
    LDA OBJ_POS_X_CONFIRMED[18],X ; Load from Xobj.
    CMP #$20 ; If _ #$20
    BCS TMP_00_POSITIVE ; >=, goto.
    LDA OBJ_POS_X_CONFIRMED[18],Y ; Load
    CMP #$E0 ; If _ #$E0
    BCC TMP_00_POSITIVE ; <, goto.
    LDA #$00 ; A seed.
    BEQ STATUS_SEEDED_A ; Always taken.
FLAGS_EQ_0x00: ; 09:19BC, 0x0139BC
    LDA OBJ_POS_X_CONFIRMED[18],X ; Load from Xobj.
    BMI ATTR_NEGATIVE ; Negative, goto.
    LDA TMP_00 ; Load
    BPL TMP_00_POSITIVE ; Positive, goto.
    LDA OBJ_POS_X_CONFIRMED[18],Y ; Load from Yobj.
    CMP #$E0 ; If _ #$E0
    BCC TMP_00_POSITIVE ; <, goto.
    LDA #$02 ; A seed.
    BNE STATUS_SEEDED_A ; Always taken.
ATTR_NEGATIVE: ; 09:19D0, 0x0139D0
    LDA TMP_00 ; Load
    BMI TMP_00_POSITIVE ; If negative, goto.
    LDA OBJ_POS_X_CONFIRMED[18],Y ; Load
    CMP #$20 ; If _ #$20
    BCS TMP_00_POSITIVE ; >=, goto.
    LDA #$01 ; A seed.
    BNE STATUS_SEEDED_A ; Always taken.
BIT_0x01_UNSET: ; 09:19DF, 0x0139DF
    LDA TMP_00 ; Load.
    BMI TMP_00_POSITIVE ; If negative, goto.
    LDA OBJ_POS_X_CONFIRMED[18],X ; Load from Xobj.
    CMP #$E0 ; If _ #$E0
    BCC TMP_00_POSITIVE ; <, goto.
    LDA OBJ_POS_X_CONFIRMED[18],Y ; Load from Yobj.
    CMP #$20 ; If _ #$20
    BCS TMP_00_POSITIVE ; >=, goto.
    LDA #$00 ; A seed.
    BEQ STATUS_SEEDED_A ; Always taken.
TMP_00_POSITIVE: ; 09:19F5, 0x0139F5
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load status.
STATUS_SEEDED_A: ; 09:19F8, 0x0139F8
    STA 556_OBJ_STATUS_FLAGS_A[18],Y ; Write Yobj status.
    LDA TMP_01 ; Load.
    CLC ; Prep add.
    ADC 4C6_OBJ_UNK[18],X ; += Xobj attr.
    STA 4C6_OBJ_UNK[18],Y ; Store to Yobj.
    LDA OBJ_POS_X??[18],X ; Load from Xobj.
    CLC ; Prep add.
    ADC #$01 ; Add.
    STA OBJ_POS_X??[18],Y ; Store to Yobj.
    LDA OBJ_POS_X_DELTA?[18],X ; Move from Xobj to Yobj.
    STA OBJ_POS_X_DELTA?[18],Y
    LDA OBJ_POS_X_SUBPIXEL_DELTA?[18],X
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],Y
    LDA #$3A
    STA OBJ_ENABLED_STATE+MORE?[18],Y ; Set Yobj.
    LDA #$18
    STA OBJ_ANIM_HOLD_TIMER?[18],X ; Set hold.
    LDA #$08
    STA 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X ; Set attr.
    LDA #$21
    JSR SND_BANKED_DISPATCH ; Play sound.
    INC 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Attr++
    LDA 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Load attr.
    CMP #$09 ; If _ #$09
    BCC EXIT_MOVE ; <, goto.
    LDA #$00
    STA 5FD_UNK ; Clear.
    JSR INIT_OBJECT[X]_DATA_FULL ; Init Xobj.
    INX ; Next Xobj.
    JMP INIT_OBJECT[X]_DATA_FULL ; Init next Xobj, abuse RTS.
EXIT_MOVE: ; 09:1A43, 0x013A43
    JMP OBJECT_X_MOVE? ; Goto, abuse RTS.
TMP_DATA_A: ; 09:1A46, 0x013A46
    .db F4
TMP_DATA_B: ; 09:1A47, 0x013A47
    .db 04
    .db 0C
    .db EC
    .db F4
    .db EC
    .db 0C
    .db 04
    .db F8
    .db F8
    .db 08
    .db F8
    .db 00
    .db F0
    .db 00
    .db 00
    .db 00
    .db F8
FILL_ATTRS_FROM_FILE: ; 09:1A58, 0x013A58
    LDA TMP_DATA_A,Y ; Seed TMP from Y index.
    STA TMP_00
    LDA TMP_DATA_B,Y
    STA TMP_01
    LDY #$00 ; Y=
    LDA 5F6_UNK ; A =
    BEQ 5F6_EQ_ZERO ; == 0, goto.
    LDY #$02 ; Alt index.
5F6_EQ_ZERO: ; 09:1A6B, 0x013A6B
    LDA [TMP_00],Y ; Load from file.
    STA OBJ_POS_X_DELTA?[18],X ; Store to obj.
    INY ; Stream++
    LDA [TMP_00],Y ; Store to OBJ.
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X
    RTS ; Leave.
TMP_DATA_A: ; 09:1A77, 0x013A77
    LOW(FILE_A)
TMP_DATA_B: ; 09:1A78, 0x013A78
    HIGH(FILE_A)
    LOW(FILE_B)
    HIGH(FILE_B)
    LOW(FILE_C)
    HIGH(FILE_C)
FILE_A: ; 09:1A7D, 0x013A7D
    .db 04
    .db 00
    .db 04
    .db 00
FILE_B: ; 09:1A81, 0x013A81
    .db 03
    .db 00
    .db 05
    .db 00
FILE_C: ; 09:1A85, 0x013A85
    .db 03
    .db 80
    .db 04
    .db 80
RTN_ATTRS_DEPENDENT_ON_TMP_10: ; 09:1A89, 0x013A89
    LDY #$00 ; Index
    LDA TMP_10 ; Load
    BEQ USE_INDEX
    LDY #$02 ; Alt index.
USE_INDEX: ; 09:1A91, 0x013A91
    LDA DATA_A,Y
    STA 503_OBJ_POS_X_LARGEST?[18],X ; From index, set.
    LDA DATA_B,Y
    STA 520_OBJ_POS_X_LARGE?[18],X
    RTS ; Leave.
DATA_A: ; 09:1A9E, 0x013A9E
    .db 00
DATA_B: ; 09:1A9F, 0x013A9F
    .db 80
    .db FF
    .db 80
OBJ_STATE_0x3A_HANDLER: ; 09:1AA2, 0x013AA2
    LDA OBJ_ANIM_HOLD_TIMER?[18],X ; Load
    LSR A ; >> 3, /8
    LSR A
    LSR A
    TAY ; To Y index.
    LDA LAST_BANK_DATA,Y ; Load data.
    STA OBJ_ANIMATION_DISPLAY[18],X ; Store animation.
    INC OBJ_ANIM_HOLD_TIMER?[18],X ; Hold++
    LDA OBJ_ANIM_HOLD_TIMER?[18],X ; Load
    CMP #$20 ; If _ #$20
    BCC EXIT_MOVE
    JMP INIT_OBJECT[X]_DATA_FULL ; Reinit object.
EXIT_MOVE: ; 09:1ABC, 0x013ABC
    JMP OBJECT_X_MOVE?
ADD_TO_VAL_5EE/5EF: ; 09:1ABF, 0x013ABF
    CLC ; Prep add.
    LDA 5EE_UNK ; Load
    ADC #$01 ; Add.
    STA 5EE_UNK ; Store back.
    LDA 5EF_UNK ; Load
    ADC #$00 ; Carry to.
    STA 5EF_UNK ; Store back.
    RTS ; Leave.
SET_ANIM_W/_HOLD/ENTRY_MOD: ; 09:1AD1, 0x013AD1
    INC OBJ_ANIM_HOLD_TIMER?[18],X ; Hold++
    LDA OBJ_ANIM_HOLD_TIMER?[18],X ; Load
    AND #$01 ; Test bit.
    BNE BIT_SET ; If odd/set, goto.
    INC 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; ++
    LDA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Load step.
    CMP #$03 ; If _ #$03
    BCC BIT_SET ; <, goto.
    LDA #$00
    STA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Reset.
BIT_SET: ; 09:1AEA, 0x013AEA
    LDY #$4E ; Y=
    LDA 5F1_UNK ; Load
    BPL Y_VAL_FOUND ; Positive, goto.
    AND #$7F ; Keep 0111.1111
    BNE Y_VAL_FOUND ; Any set, goto.
    LDA 5F0_UNK ; Load other val.
    CMP #$04 ; If _ #$04
    BCS Y_VAL_FOUND ; >=, goto.
    LDY #$53 ; Alt Y val.
Y_VAL_FOUND: ; 09:1AFE, 0x013AFE
    TYA ; To A.
    CLC ; Prep add.
    ADC 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Add to.
    STA OBJ_ANIMATION_DISPLAY[18],X ; Store as animation.
    RTS ; Leave.
OBJ_STATE_0x37_HANDLER: ; 09:1B07, 0x013B07
    LDA OBJ_SECONDARY_SWITCH?[18],X ; Get secondary.
    BNE SECONDARY_NONZERO
    LDA #$51
    STA OBJ_ANIMATION_DISPLAY[18],X ; Set animation if zero.
SECONDARY_NONZERO: ; 09:1B11, 0x013B11
    LDA #$00
    STA 4C6_OBJ_UNK[18],X ; Clear
    LDA OBJ_POS_X_SUBPIXEL?+17,X ; Move to Xobj. TODO: Why 17th object?
    STA OBJ_POS_X??[18],X
    LDA OBJ_POS_Y_CONFIRMED+17,X ; Move from Obj[17]
    STA OBJ_POS_X_CONFIRMED[18],X
    LDA 544_OBJ_UNK_POS_DELTA?+17,X ; Move from Obj[17]
    STA 556_OBJ_STATUS_FLAGS_A[18],X
    JMP OBJECT_X_MOVE? ; Goto, abuse RTS.
OBJ_STATE_0x38_HANDLER: ; 09:1B2B, 0x013B2B
    LDA OBJ_SECONDARY_SWITCH?[18],X ; Load secondary.
    CMP #$01 ; If _ #$01
    BNE SWITCH_SECONDARY ; !=, goto.
    TXA ; Xobj to A.
    LSR A ; >> 1, /2.
    BCC OBJECT_EVEN ; If even, goto.
    LDA IRQ/SCRIPT_RUN_COUNT? ; Load
    LSR A ; >> 1, /2.
    BCC SWITCH_SECONDARY ; If even, goto.
    BCS EXTRA_SUBRTN ; If odd, goto.
OBJECT_EVEN: ; 09:1B3D, 0x013B3D
    LDA IRQ/SCRIPT_RUN_COUNT? ; Load
    LSR A ; >> 1, /2
    BCS SWITCH_SECONDARY
EXTRA_SUBRTN: ; 09:1B42, 0x013B42
    JSR OBJ_RTN_UNK_RET_VAL_UNK ; Do sub.
SWITCH_SECONDARY: ; 09:1B45, 0x013B45
    LDA OBJ_SECONDARY_SWITCH?[18],X ; Switch on.
    JSR SWITCH_CODE_PTRS_PAST_JSR
    LOW(SWITCH_RTN_A)
    HIGH(SWITCH_RTN_A)
    LOW(SWITCH_RTN_B)
    HIGH(SWITCH_RTN_B)
    LOW(SWITCH_RTN_C)
    HIGH(SWITCH_RTN_C)
SWITCH_RTN_A: ; 09:1B51, 0x013B51
    LDA #$FF
    STA OBJECT_DATA_HEALTH?[18],X ; Set attr.
    LDA #$AA
    STA OBJECT_DATA_EXTRA_B?[18],X ; Set extra.
    LDA #$56
    STA OBJ_ANIMATION_DISPLAY[18],X ; Set anim.
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load status.
    AND #$04 ; Keep bit.
    TAY ; To Y index.
    LDA OBJ_DATA_A,Y ; Load data.
    CLC ; Prep add.
    ADC OBJ_POS_X_DELTA? ; Add with.
    STA OBJ_POS_X_DELTA?[18],X ; Store to Xobj.
    LDA OBJ_DATA_B,Y ; Load
    CLC ; Prep add.
    ADC OBJ_POS_X_SUBPIXEL_DELTA? ; Add with.
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X ; Store attr.
    LDA OBJ_DATA_C,Y
    STA 532_OBJ_UNK_POS_DELTA?[18],X ; Set from data.
    LDA OBJ_DATA_D,Y
    STA 544_OBJ_UNK_POS_DELTA?[18],X ; Set from data.
    INC OBJ_SECONDARY_SWITCH?[18],X ; Move secondary.
    JSR OBJECT_X_MOVE? ; Do.
    LDA #$0C
    JMP SND_BANKED_DISPATCH ; Play sound. Abuse RTS.
OBJ_DATA_A: ; 09:1B8F, 0x013B8F
    .db FD
OBJ_DATA_B: ; 09:1B90, 0x013B90
    .db 00
OBJ_DATA_C: ; 09:1B91, 0x013B91
    .db 03
OBJ_DATA_D: ; 09:1B92, 0x013B92
    .db 00
    .db 03
    .db 00
    .db 03
    .db 00
SWITCH_RTN_B: ; 09:1B97, 0x013B97
    JSR MOVE_OBJ_ATTR_BY_DELTA_UNK ; Do.
    BMI EXIT_RTN_EXTRA ; Ret negative, goto.
    LDA #$10
    STA 59E_OBJ_UNK/EXTRA_TIMER[18],X ; Set attr.
    INC OBJ_SECONDARY_SWITCH?[18],X ; Move secondary.
EXIT_RTN_EXTRA: ; 09:1BA4, 0x013BA4
    JSR OBJECT_X_MOVE? ; Do.
    JSR OBJ_POS_BASED_INIT_CLEAR ; Do.
    LDA 556_OBJ_STATUS_FLAGS_A[18],X ; Load
    AND #$03 ; Keep 0000.0011
    BEQ EXIT_NO_INIT
    JMP INIT_OBJECT[X]_DATA_FULL ; Any set, init. Abuse RTS.
EXIT_NO_INIT: ; 09:1BB4, 0x013BB4
    RTS ; None set, RTS.
SWITCH_RTN_C: ; 09:1BB5, 0x013BB5
    JSR CLEAR_OBJ_ATTRS_UNK_1546 ; Clear.
    JSR CLEAR_OBJ_ATTRS_DELTAS? ; Clear.
    LDA #$57
    STA OBJ_ANIMATION_DISPLAY[18],X ; Set animation.
    LDA 59E_OBJ_UNK/EXTRA_TIMER[18],X ; Load
    CMP #$08 ; If _ #$08
    BCS DONT_INC_ANIMATION ; >=, goto.
    INC OBJ_ANIMATION_DISPLAY[18],X ; Next animation.
DONT_INC_ANIMATION: ; 09:1BCA, 0x013BCA
    DEC 59E_OBJ_UNK/EXTRA_TIMER[18],X ; --
    BNE NOT_EQ_ZERO ; != 0, goto.
    JMP INIT_OBJECT[X]_DATA_FULL ; Exit with init.
NOT_EQ_ZERO: ; 09:1BD2, 0x013BD2
    JMP EXIT_RTN_EXTRA ; Exit move.
OBJ_STATE_0x39_HANDLER: ; 09:1BD5, 0x013BD5
    LDA OBJ_SECONDARY_SWITCH?[18],X
    JSR SWITCH_CODE_PTRS_PAST_JSR
    LOW(STATE_0x39_SUBSWITCH_A) ; Setup.
    HIGH(STATE_0x39_SUBSWITCH_A)
    LOW(STATE_0x39_SUBSWITCH_B) ; Attrs
    HIGH(STATE_0x39_SUBSWITCH_B)
    LOW(STATE_0x39_SUBSWITCH_C) ; Weird.
    HIGH(STATE_0x39_SUBSWITCH_C)
STATE_0x39_SUBSWITCH_A: ; 09:1BE1, 0x013BE1
    LDA #$FF
    STA OBJECT_DATA_HEALTH?[18],X ; Set health.
    LDA #$AB
    STA OBJECT_DATA_EXTRA_B?[18],X ; Set attr.
    LDA #$52
    STA OBJ_ANIMATION_DISPLAY[18],X ; Set anim.
    LDA OBJ_POS_X_DELTA? ; Move RAM to Xobj.
    STA OBJ_POS_X_DELTA?[18],X
    LDA OBJ_POS_X_SUBPIXEL_DELTA?
    STA OBJ_POS_X_SUBPIXEL_DELTA?[18],X
    INC OBJ_SECONDARY_SWITCH?[18],X ; Move secondary.
    JMP OBJECT_X_MOVE? ; Goto, abuse RTS.
STATE_0x39_SUBSWITCH_B: ; 09:1C00, 0x013C00
    LDA #$20
    LDY #$00
    JSR ADD_A/Y_TO_DELTA_UNK_C ; Mod.
    JSR MOVE_OBJ_ATTR_BY_DELTA_UNK ; Move.
    BMI EXIT_INIT_POSSIBLE ; If negative, goto.
    JSR CLEAR_OBJ_ATTRS_UNK_1546 ; Clear attrs.
    JSR CLEAR_OBJ_ATTRS_DELTAS?
    INC OBJ_SECONDARY_SWITCH?[18],X ; Move secondary.
    LDA #$21
    JSR SND_BANKED_DISPATCH ; Play sound.
EXIT_INIT_POSSIBLE: ; 09:1C1A, 0x013C1A
    JMP EXIT_RTN_EXTRA ; Do. Abuse RTS.
STATE_0x39_SUBSWITCH_C: ; 09:1C1D, 0x013C1D
    TXA ; Xobj to A.
    LSR A ; >> 1, /2.
    BCC OBJ_EVEN ; If even goto.
    LDA IRQ/SCRIPT_RUN_COUNT? ; Load
    LSR A ; >> 1, /2.
    BCC SHIFT_CLEAR
    BCS SHIFT_SET
OBJ_EVEN: ; 09:1C28, 0x013C28
    LDA IRQ/SCRIPT_RUN_COUNT? ; Load
    LSR A ; >> 1, /2.
    BCS SHIFT_CLEAR ; Shifted set, do opposite of other rtn.
SHIFT_SET: ; 09:1C2D, 0x013C2D
    JSR OBJ_RTN_UNK_RET_VAL_UNK ; Do.
SHIFT_CLEAR: ; 09:1C30, 0x013C30
    LDA #$AC
    STA OBJECT_DATA_EXTRA_B?[18],X ; Set.
    INC OBJ_ANIM_HOLD_TIMER?[18],X ; ++
    LDA OBJ_ANIM_HOLD_TIMER?[18],X ; Load
    CMP #$06 ; If _ #$06
    BCC RTN ; <, goto.
    LDA #$00
    STA OBJ_ANIM_HOLD_TIMER?[18],X ; Clear hold.
    INC 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; ++
    LDA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Load
    CMP #$03 ; If _ #$03
    BCC RTN ; <, goto.
    JMP INIT_OBJECT[X]_DATA_FULL ; Goto, abuse RTS.
RTN: ; 09:1C51, 0x013C51
    LDA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Load
    CLC ; Prep add.
    ADC #$01 ; Add.
    STA OBJ_ANIMATION_DISPLAY[18],X ; Set anim.
    JMP EXIT_RTN_EXTRA ; Exit. Abuse RTS.
LAST_BANK_DATA: ; 09:1C5D, 0x013C5D
    .db 01
    .db 02
    .db 03
    .db 03
    .db FF ; End of bank data.
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db FF
    .db 29
