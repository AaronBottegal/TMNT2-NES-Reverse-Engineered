...
EOL_CUTSCENE?: ; 12:0001, 0x024001
    ...
    LDA #$FF
    STA 639_SWITCH_EOL_SCRIPT_IN_PROGRESS? ; Set to timer.
    LDA #$00
...
LEVEL_TIMER_FINISHED: ; 12:002C, 0x02402C
    ...
    STA 639_SWITCH_EOL_SCRIPT_IN_PROGRESS? ; Clear switch.
    SEC ; Set carry, finished.
RTS: ; 12:0040, 0x024040
    RTS ; Leave.
EOL_SCRIPT_SWITCH: ; 12:0041, 0x024041
    LDA 639_SWITCH_EOL_SCRIPT_IN_PROGRESS? ; Switch on.
...
SWITCH_RTN_A: ; 12:008B, 0x02408B
    ...
    STA 639_SWITCH_EOL_SCRIPT_IN_PROGRESS? ; Store to switch.
    TYA ; To A.
...
SWITCH_RTN_G: ; 12:01E9, 0x0241E9
    ...
    STA TMP_0E
    JSR 12:18A0
L_12:0231: ; 12:0231, 0x024231
    RTS
L_12:0232: ; 12:0232, 0x024232
    LDA #$FF
...
SWITCH_RTN_E: ; 12:023C, 0x02423C
    ...
    STA OBJ_SECONDARY_SWITCH?[18]
    STA OBJ_SECONDARY_SWITCH?+2
    STA 5C2_OBJ_DATA_PTR/MISC_INDEX[18]
    STA 5C2_OBJ_DATA_PTR/MISC_INDEX+2
    LDA #$08
...
P1_GAME_OVER: ; 12:02F4, 0x0242F4
    LDX #$02 ; Obj.
    LDA TWO_PLAYERS_FLAG ; Load to test.
...
L_12:0407: ; 12:0407, 0x024407
    ...
    INC 639_SWITCH_EOL_SCRIPT_IN_PROGRESS?
    JSR INIT_OBJECT[X]_DATA_FULL
    JSR PREP_OBJS_0-3
L_12:0429: ; 12:0429, 0x024429
    RTS
SWITCH_RTN_D: ; 12:042A, 0x02442A
    ...
    JSR WRITE_PPU_COPY_TO_CTRL
    LDA #$01
    JSR RTN_PALETTE_MOD_AND_UPLOAD
    LDA #$0A
    STA OBJ_ENABLED_STATE+MORE?+2
    LDA #$90
    STA OBJ_POS_X_CONFIRMED+2
    LDA #$80
    STA OBJ_POS_Y_CONFIRMED+2
    LDA #$00
    STA OBJ_STATE_DIR_RELATED_C_SPR_DATA?+2
    LDA #$70
    STA OBJ_ANIMATION_DISPLAY+2
    LDX #$00
    LDA NUM_PLAYER_LIVES[2]
...
SWITCH_RTN_B: ; 12:0537, 0x024537
    ...
    SEC
    LDA 12:012E
    SBC OBJ_POS_X_CONFIRMED[18],X
    BPL L_12:054F
    EOR #$FF
    CLC
    ADC #$01
L_12:054F: ; 12:054F, 0x02454F
    CMP #$30
    BCS L_12:0578
    SEC
    LDA 12:012F
...
SWITCH_RTN_G: ; 12:05CE, 0x0245CE
    ...
    STA TIMER_GAME_SCRIPT?
    LDA #$10
    STA 639_SWITCH_EOL_SCRIPT_IN_PROGRESS?
    JMP PREP_OBJS_0-3
L_12:05F3: ; 12:05F3, 0x0245F3
    LDA #$04
    STA OBJ_TERTIARY_SWITCH?[18],X
L_12:05F8: ; 12:05F8, 0x0245F8
    RTS
    TXA
    LSR A
    TAY
...
SWITCH_RTN_P: ; 12:06A7, 0x0246A7
    ...
    DEC TIMER_GAME_SCRIPT?
    BNE L_12:06BC
L_12:06B1: ; 12:06B1, 0x0246B1
    JSR PSWAP_FADEOUT_TEXT
    LDA PALETTE_SWAP_QUEUEING_IN_PROGRESS
    BNE L_12:06BC
    INC 639_SWITCH_EOL_SCRIPT_IN_PROGRESS?
L_12:06BC: ; 12:06BC, 0x0246BC
    RTS
...
L_12:0767: ; 12:0767, 0x024767
    RTS
L_12:0768: ; 12:0768, 0x024768
    INC 639_SWITCH_EOL_SCRIPT_IN_PROGRESS?
    RTS
...
PREP_OBJS_0-3: ; 12:0D0E, 0x024D0E
    LDX #$00
    JSR INIT_OBJECT[X]_DATA_FULL ; Init 0-3
    INX
    JSR INIT_OBJECT[X]_DATA_FULL
    INX
    JSR INIT_OBJECT[X]_DATA_FULL
    ...
    CLC
    ADC 544_OBJ_UNK_POS_DELTA?+1,X
    STA 544_OBJ_UNK_POS_DELTA?+1,X
    BCC L_12:0D2B
    INC 532_OBJ_UNK_POS_DELTA?+1,X
L_12:0D2B: ; 12:0D2B, 0x024D2B
    CLC
    LDA 544_OBJ_UNK_POS_DELTA?+1,X
    ADC 4D8_OBJ_UNK[18],X
    STA 4D8_OBJ_UNK[18],X
    LDA 532_OBJ_UNK_POS_DELTA?+1,X
    ADC 4C6_OBJ_UNK[18],X
    STA 4C6_OBJ_UNK[18],X
    BPL L_12:0D43
    CLC
    BMI L_12:0D44
L_12:0D43: ; 12:0D43, 0x024D43
    SEC
L_12:0D44: ; 12:0D44, 0x024D44
    ...
    BCS L_12:0D51
    ORA #$40
L_12:0D51: ; 12:0D51, 0x024D51
    STA OBJ_STATE_DIR_RELATED_C_SPR_DATA?[18],X
    RTS
RTN_PALETTE_MOD_AND_UPLOAD: ; 12:0D55, 0x024D55
    JSR PALETTE_MOD_FROM_FILE ; Mod from file number.
    TXA ; Save X.
    PHA
    LDA #$05 ; File 5. Custom RAM data from $06D7.
...
PALETTE_DATA_PTRS_LOW: ; 12:0D9C, 0x024D9C
    LOW(PALETTE_DATA_A) ; 0x00
PALETTE_DATA_PTRS_HIGH: ; 12:0D9D, 0x024D9D
    HIGH(PALETTE_DATA_A)
    LOW(PALETTE_DATA_A) ; 0x01
    HIGH(PALETTE_DATA_A)
    LOW(PALETTE_DATA_B) ; 0x02
    HIGH(PALETTE_DATA_B)
    LOW(PALETTE_DATA_C) ; 0x03
    HIGH(PALETTE_DATA_C)
    LOW(PALETTE_DATA_D) ; 0x04
    HIGH(PALETTE_DATA_D)
    LOW(PALETTE_DATA_E) ; 0x05
    HIGH(PALETTE_DATA_E)
    LOW(PALETTE_DATA_F) ; 0x06
    HIGH(PALETTE_DATA_F)
    LOW(PALETTE_DATA_10) ; 0x07
    HIGH(PALETTE_DATA_10)
    LOW(PALETTE_DATA_11) ; 0x08
    HIGH(PALETTE_DATA_11)
    LOW(PALETTE_DATA_12) ; 0x09
    HIGH(PALETTE_DATA_12)
    LOW(PALETTE_DATA_13) ; 0x0A
    HIGH(PALETTE_DATA_13)
    LOW(PALETTE_DATA_14) ; 0x0B
    HIGH(PALETTE_DATA_14)
    LOW(PALETTE_DATA_15) ; 0x0C
    HIGH(PALETTE_DATA_15)
    LOW(PALETTE_DATA_16) ; 0x0D
    HIGH(PALETTE_DATA_16)
    LOW(PALETTE_DATA_17) ; 0x0E
    HIGH(PALETTE_DATA_17)
    LOW(PALETTE_DATA_18) ; 0x0F
    HIGH(PALETTE_DATA_18)
    LOW(PALETTE_DATA_19) ; 0x0F
    HIGH(PALETTE_DATA_19)
    LOW(PALETTE_DATA_1A) ; 0x10
    HIGH(PALETTE_DATA_1A)
    LOW(PALETTE_DATA_1B) ; 0x11
    HIGH(PALETTE_DATA_1B)
    LOW(PALETTE_DATA_1C) ; 0x12
...
P2_ANIMATION: ; 12:1076, 0x025076
    STA SCRIPT_R0-R5_GFX_BANK_VALS+3 ; Store to R3.
...
VAL_EQ_FF: ; 12:107F, 0x02507F
    ...
    LDA #$00 ; Clear
    STA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Clear entry.
...
RESET_STREAM_RESET_DATA: ; 12:1092, 0x025092
    LDA #$00
    STA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Reset entry on.
...
CUTSCENE_SCENE_1: ; 12:1416, 0x025416
    ...
    JSR SOUND_RESET/INIT_RTN? ; Init sound.
...
SPRITE_OBJECT_FILLOUT_LOOP: ; 12:1468, 0x025468
    JSR INIT_OBJECT[X]_DATA_FULL ; Init object X.
    LDA #$01
...
LOOP_ALL_OBJECTS: ; 12:151C, 0x02551C
    ...
    JSR XPOS_MOD_DELTA_RET_CS_WRAPAROUND ; Do sub.
    LDA #$18 ; Move val?
    JSR MOVE_UNK_RET_CS_POS_CS_NEG_ADD_USE
    JSR MOVE_Y_FINALIZE
...
ALL_VALUES_ZERO?: ; 12:1563, 0x025563
    ...
    LDX #$06
    JSR INIT_OBJECT[X]_DATA_FULL ; Init 7th object.
    LDA #$04
    STA DISABLE_RENDERING_X_FRAMES ; Disable rendering.
    JSR 63E_SWITCH_MOVE ; State++
L_12:1572: ; 12:1572, 0x025572
    RTS
...
INTRO_CUTSCENE_RTN_E: ; 12:1588, 0x025588
    LDA #$C5
    STA SCRIPT_R0-R5_GFX_BANK_VALS+2 ; Set gfx R2-R3
    LDA #$C6
    STA SCRIPT_R0-R5_GFX_BANK_VALS+3
    LDA #$EC
    STA SCRIPT_R0-R5_GFX_BANK_VALS[6] ; Set gfx R0-R1
    LDA #$EE
    STA SCRIPT_R0-R5_GFX_BANK_VALS+1
    LDX #$0E
    JSR BANK_PAIR_SAVE+PPU_FILE_BANK_14/15 ; Upload screen.
    LDA #$05
    JSR RTN_PALETTE_MOD_AND_UPLOAD ; Palette?
    LDX #$00 ; Object
...
LOOP_ALL_OBJS: ; 12:1600, 0x025600
    ...
    LDA 5C2_OBJ_DATA_PTR/MISC_INDEX[18] ; Load
    ORA 5C2_OBJ_DATA_PTR/MISC_INDEX+1 ; Or
    ORA 5C2_OBJ_DATA_PTR/MISC_INDEX+2 ; Or
    ORA 5C2_OBJ_DATA_PTR/MISC_INDEX+3 ; Or
...
INTRO_FALLING_RTN_B: ; 12:1658, 0x025658
    ...
    STA SCRIPT_R0-R5_GFX_BANK_VALS+2,X ; Store new bank in R2-R5.
...
INTRO_FALLING_RTN_C: ; 12:16B1, 0x0256B1
    DEC 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X ; Timer
    BNE L_12:16B9
    INC OBJ_SECONDARY_SWITCH?[18],X
L_12:16B9: ; 12:16B9, 0x0256B9
    RTS
INTRO_FALLING_RTN_D: ; 12:16BA, 0x0256BA
    JSR XPOS_MOD_DELTA_RET_CS_WRAPAROUND ; Move
...
NOT_FINISHED: ; 12:16D1, 0x0256D1
    ...
    STA SCRIPT_R0-R5_GFX_BANK_VALS+2,X ; Switch in bank.
    RTS ; Leave.
...
LOOP: ; 12:1723, 0x025723
    ...
    STA PPU_UPDATE_BUFFER[64],X ; To update buffer.
    INX ; Update buffer++
    INY ; Stream++
...
VAL_NEG: ; 12:19B8, 0x0259B8
    LDA STATUS_FLAGS_B/OBJ_FOCUS[18],X ; Load val.
...
RTN_TEXT_AFTER_CUTSCENE?: ; 12:19FA, 0x0259FA
    ...
    LOW(63B_RTN_A_SETUP) ; Screen setup?
    HIGH(63B_RTN_A_SETUP)
    LOW(63B_RTN_B_TEXT_SPRITES) ; Init text sprite objs.
    HIGH(63B_RTN_B_TEXT_SPRITES)
    LOW(63B_RTN_C_TEXT_CONTINUE) ; Continue and wait for all to complete.
    HIGH(63B_RTN_C_TEXT_CONTINUE)
...
DATA_UNK: ; 12:1A15, 0x025A15
    ...
    .db 00
    .db 00
63B_RTN_A_SETUP: ; 12:1A25, 0x025A25
    LDA #$03
    STA 3D5_UNK ; Store to.
    LDA #$05
    STA DISABLE_RENDERING_X_FRAMES ; Disable rendering.
    INC SWITCH_TEXT_INTRO ; Switch++
    LDA #$03
    JSR RTN_PALETTE_MOD_AND_UPLOAD ; Do palette.
    JMP CLEAR_IRQ_FLAGS_UNK
63B_RTN_B_TEXT_SPRITES: ; 12:1A39, 0x025A39
    JSR DISPATCH_CLEAR_SCREEN ; Clear screen.
    LDA LEVEL/SCREEN_ON ; Load index.
    JSR INIT_GFX_BANKS+OBJ_STREAMS_TEXT ; Do sub.
    INC SWITCH_TEXT_INTRO ; Next state.
    RTS
63B_RTN_C_TEXT_CONTINUE: ; 12:1A45, 0x025A45
...
PREP_TEXT_FADE_RTN: ; 12:1A5D, 0x025A5D
    ...
    STA SWITCH_TEXT_INTRO ; Prep flag.
...
INIT_GFX_BANKS+OBJ_STREAMS_TEXT: ; 12:1AB9, 0x025AB9
    ...
    LDA #$68
    STA SCRIPT_R0-R5_GFX_BANK_VALS[6] ; Set GFX banks R0-R1
    LDA #$6A
    STA SCRIPT_R0-R5_GFX_BANK_VALS+1
    LDA #$68
    STA SCRIPT_R0-R5_GFX_BANK_VALS+2 ; Also set R2-R5.
    LDA #$69
    STA SCRIPT_R0-R5_GFX_BANK_VALS+3
    LDA #$6A
    STA SCRIPT_R0-R5_GFX_BANK_VALS+4
    ...
    AND #$DF
    STA PPU_CTRL_RAM_COPY ; Store CTRL.
    LDA #$80
...
RTN_FORWARD_OBJ+?_RET_CS_COMPLETED: ; 12:1B38, 0x025B38
    LDY 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Load from object. NOTE: Carry set means completed?
LOOP_KEEP_Y_ASIS: ; 12:1B3B, 0x025B3B
    LDA 5D4_EXTRA_TIMER/OBJ/FOCUS[18],X ; Load timer?
...
REENTER_RTN_??: ; 12:1B46, 0x025B46
    ...
    LDA OBJ_POS_X_CONFIRMED[18],X ; Setup temp..?
    STA TMP_00
    LDA OBJ_POS_X??[18],X
    STA TMP_01
...
SUB_NOT_READY_FWD: ; 12:1B7A, 0x025B7A
    CLC ; Clear.
    TYA ; Y to A.
    STA 5C2_OBJ_DATA_PTR/MISC_INDEX[18],X ; Store index back to object.
    RTS ; Leave.
...
RTN_BG_UPDATES_TEXT_FADE_IN?: ; 12:1C14, 0x025C14
    ...
    LDA #$04
    STA PPU_UPDATE_BUFFER[64],X ; Config?
    LDA #$10
...
LOOP_WHOLE_PALETTE: ; 12:1C30, 0x025C30
    ...
    AND #$03 ; Keep bits.
    TAY ; To Y index.
    LDA 45A_OBJ_DATA_ENTRY?STATE_STEP?+7,Y ; Load from obj 7-10
    ASL A ; << 4, *16.
    ASL A
    ASL A
    ASL A
...
LOOP_TO_OBJ_18: ; 12:1C75, 0x025C75
    JSR INIT_OBJECT[X]_DATA_FULL ; Init object.
    CPX #$0B ; Object 12?
    BCS OBJ_PAST_CHECK ; >=, goto. Don't set values.
    LDA #$04
    STA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],X ; Set to 4.
    TYA
    STA OBJ_STATE_DIR_RELATED_C_SPR_DATA?[18],X ; Y val to here.
    INY ; Y++
...
RTN_UPDATES_BG?: ; 12:1CAE, 0x025CAE
    ...
    PHA ; Save
    JSR INIT_OBJECT[X]_DATA_FULL ; Init obj X.
    PLA ; Pull
    STA OBJ_STATE_DIR_RELATED_C_SPR_DATA?[18],X ; Restore.
...
RTN_MAKE_OBJ_RET_CS_FAIL: ; 12:1CE3, 0x025CE3
    ...
    ASL A
    ASL A
    STA OBJ_POS_X_CONFIRMED[18],Y ; Store to object.
    LDA TMP_01 ; Load val.
    ASL A ; << 3, *8
    ASL A
    ASL A
    ...
    STA OBJ_POS_Y_CONFIRMED[18],Y ; Also here.
    LDA TMP_02 ; Load val.
    STA 5D4_EXTRA_TIMER/OBJ/FOCUS[18],Y ; Store.
    JSR RTN_MODIFY_A_???
    STA OBJ_ANIMATION_DISPLAY[18],Y ; Store to anim?
    LDA #$03
    STA 45A_OBJ_DATA_ENTRY?STATE_STEP?[18],Y ; Set val.
    LDA 3D5_UNK
...
TEXT?_PTR_L: ; 12:1D2D, 0x025D2D
    LOW(TEXT_FILE_A) ; 0x00
TEXT?_PTR_H: ; 12:1D2E, 0x025D2E
    HIGH(TEXT_FILE_A)
    LOW(TEXT_FILE_B) ; 0x01
    HIGH(TEXT_FILE_B)
    LOW(TEXT_FILE_B) ; 0x02
    HIGH(TEXT_FILE_B)
    LOW(TEXT_FILE_C) ; 0x03
    HIGH(TEXT_FILE_C)
    LOW(TEXT_FILE_C) ; 0x04
    HIGH(TEXT_FILE_C)
    LOW(TEXT_FILE_D) ; 0x05
    HIGH(TEXT_FILE_D)
    LOW(TEXT_FILE_D) ; 0x06
    HIGH(TEXT_FILE_D)
    LOW(TEXT_FILE_E) ; 0x07
    HIGH(TEXT_FILE_E)
    LOW(TEXT_FILE_E) ; 0x08
    HIGH(TEXT_FILE_E)
    LOW(TEXT_FILE_F) ; 0x09
    HIGH(TEXT_FILE_F)
    LOW(TEXT_FILE_G) ; 0x0A
    HIGH(TEXT_FILE_G)
    LOW(TEXT_FILE_H) ; 0x0B
    HIGH(TEXT_FILE_H)
    LOW(TEXT_FILE_H) ; 0x0C
    HIGH(TEXT_FILE_H)
    LOW(TEXT_FILE_H) ; 0x0D
    HIGH(TEXT_FILE_H)
    LOW(TEXT_FILE_H) ; 0x0E
    HIGH(TEXT_FILE_H)
