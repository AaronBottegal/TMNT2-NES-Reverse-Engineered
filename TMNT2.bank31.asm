...
60A_EQ_ZERO: ; 1F:0F0D, 0x03EF0D
    ...
    LDA #$FF
    STA BG_UPDATING_FLAG_UNK ; Set to 0xFF.
...
UNK_OTHER: ; 1F:0F1F, 0x03EF1F
    ...
    BNE VAL_NOT_FF ; !=, goto.
    LDA #$00
    STA BG_UPDATING_FLAG_UNK ; Clear if not FF.
...
OBJ_HANDLERS_NEGATIVE_H: ; 1F:122A, 0x03F22A
    HIGH(OBJ_STATE_0x80_HANDLER) ; Bowling ball creator.
    LOW(OBJ_STATE_0x81_HANDLER) ; 0x01
    HIGH(OBJ_STATE_0x81_HANDLER) ; Bowling ball object.
    LOW(OBJ_STATE_0x82_HANDLER) ; 0x02
    HIGH(OBJ_STATE_0x82_HANDLER) ; BG updating handler.
    ...
    HIGH(OBJ_STATE_0x8D_HANDLER) ; BG Update.
    LOW(OBJ_STATE_0x8E_HANDLER) ; 0x0E
    HIGH(OBJ_STATE_0x8E_HANDLER) ; BG Update.
    LOW(OBJ_STATE_0x8F_HANDLER) ; 0x0F
    HIGH(OBJ_STATE_0x8F_HANDLER) ; BG Update move.
    LOW(OBJ_STATE_0x90_HANDLER) ; 0x10
    HIGH(OBJ_STATE_0x90_HANDLER) ; BG Update flagged.
    LOW(OBJ_STATE_0x90_HANDLER) ; 0x11
    HIGH(OBJ_STATE_0x90_HANDLER)
    LOW(OBJ_STATE_0x92_HANDLER) ; 0x12
    HIGH(OBJ_STATE_0x92_HANDLER) ; Uses bossy, giant shooting thingy.
    ...
    HIGH(OBJ_STATE_0xA4_HANDLER) ; BG Update move?
    LOW(OBJ_STATE_0xA5_HANDLER) ; 0x25
    HIGH(OBJ_STATE_0xA5_HANDLER) ; BG Update.
    LOW(OBJ_STATE_0xA6_HANDLER) ; 0x26
    HIGH(OBJ_STATE_0xA6_HANDLER) ; BG Update.
    ...
    HIGH(OBJ_STATE_0xAF_HANDLER) ; Just die already, lol.
    ...
    HIGH(OBJ_STATE_0xB2_HANDLER) ; BG Update move.
    ...
    HIGH(OBJ_STATE_0xBB_HANDLER) ; Simple, move to other.
    ...
    HIGH(OBJ_STATE_0xC0_HANDLER) ; BG update medium, TODO.
...
BIT_0x02_SET: ; 1F:13E6, 0x03F3E6
    LDA OBJ_POS_X_CONFIRMED[18],X ; Load
    CMP #$D0 ; If _ #$D0
    BCC EXIT_DIE ; <, exit reinit.
RTS: ; 1F:13ED, 0x03F3ED
    RTS ; Leave.
EXIT_DIE: ; 1F:13EE, 0x03F3EE
...
SUB_INVERT_X_DELTA?: ; 1F:1576, 0x03F576
    ...
    RTS ; Leave.
ADD_A/Y_TO_DELTA_UNK_A: ; 1F:1588, 0x03F588
    STY TMP_00
    CLC
    ...
    STA OBJ_POS_X_DELTA?[18],X
    RTS
ADD_A/Y_TO_DELTA_UNK_B: ; 1F:159A, 0x03F59A
    STY TMP_00 ; Store Y to.
    CLC ; Prep add.
    ADC 520_OBJ_POS_X_LARGE?[18],X ; Add A passed with.
    STA 520_OBJ_POS_X_LARGE?[18],X ; Store back.
    LDA 503_OBJ_POS_X_LARGEST?[18],X ; Load
    ADC TMP_00 ; Add with Y passed.
    STA 503_OBJ_POS_X_LARGEST?[18],X ; Store to.
    RTS ; Leave.
ADD_A/Y_TO_DELTA_UNK_C: ; 1F:15AC, 0x03F5AC
    STY TMP_00 ; Y to TMP.
    CLC ; Prep add.
    ...
    RTS
MOVE_OBJ_ATTR_BY_DELTA_UNK: ; 1F:15BE, 0x03F5BE
    CLC ; Prep add.
...
MOVE_UNK_RET_??: ; 1F:1895, 0x03F895
    ...
    ASL A ; Shift back.
    RTS ; Leave.
BG_UPDATE_DATA_A: ; 1F:18AE, 0x03F8AE
    .db 02
    .db 03
    .db 04
    .db 05
    .db 01
    .db 00
BG_UPDATE_DATA_B: ; 1F:18B4, 0x03F8B4
    .db 09
    .db 0C
    .db 11
    .db 0A
    ...
    .db 15
    .db 0B
BG_UPDATE_DATA_C: ; 1F:18C1, 0x03F8C1
    .db 17
    .db 18
    .db 17
    .db 1C
    .db 1B
BG_UPDATE_DATA_D: ; 1F:18C6, 0x03F8C6
    .db 1E
    .db 1E
    .db 25
    .db 26
    .db 27
BG_UPDATE_DATA_E: ; 1F:18CB, 0x03F8CB
    .db 29
    .db 29
    .db 29
    .db 2A
    .db 2B
    .db 2C
BG_UPDATE_DATA_F: ; 1F:18D1, 0x03F8D1
    .db 2D
    .db 2E
    .db 2F
    .db 30
...
OBJ_DATA_A_LOW: ; 1F:18D6, 0x03F8D6
    LOW(BG_UPDATE_DATA_A)
OBJ_DATA_A_HIGH: ; 1F:18D7, 0x03F8D7
    HIGH(BG_UPDATE_DATA_A)
    LOW(BG_UPDATE_DATA_A)
    HIGH(BG_UPDATE_DATA_A)
    LOW(BG_UPDATE_DATA_B)
    HIGH(BG_UPDATE_DATA_B)
    LOW(BG_UPDATE_DATA_C)
    HIGH(BG_UPDATE_DATA_C)
    LOW(OBJ_DATA_A_LOW) ; Pre-named?
    HIGH(OBJ_DATA_A_LOW)
    LOW(BG_UPDATE_DATA_D)
    HIGH(BG_UPDATE_DATA_D)
    LOW(BG_UPDATE_DATA_E)
    HIGH(BG_UPDATE_DATA_E)
    LOW(BG_UPDATE_DATA_E)
    HIGH(BG_UPDATE_DATA_E)
    LOW(BG_UPDATE_DATA_F)
    HIGH(BG_UPDATE_DATA_F)
    LOW(OBJ_DATA_A_LOW) ; Pre-named?
    HIGH(OBJ_DATA_A_LOW)
    LOW(BG_UPDATE_DATA_F)
    HIGH(BG_UPDATE_DATA_F)
